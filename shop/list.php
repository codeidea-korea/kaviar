...
define('_SHOPLIST_', true); //인태 - 추가

$ca_id = isset($_REQUEST['ca_id']) ? safe_replace_regex($_REQUEST['ca_id'], 'ca_id') : '';
$ca_id2 = isset($_REQUEST['ca_id2']) ? safe_replace_regex($_REQUEST['ca_id2'], 'ca_id2') : '';
$ca_id3 = isset($_REQUEST['ca_id3']) ? safe_replace_regex($_REQUEST['ca_id3'], 'ca_id3') : '';
$skin = isset($_REQUEST['skin']) ? safe_replace_regex($_REQUEST['skin'], 'skin') : '';

// 상품 리스트에서 다른 필드로 정렬을 하려면 아래의 배열 코드에서 해당 필드를 추가하세요.
if( isset($sort) && ! in_array($sort, array('it_name', 'it_sum_qty', 'it_price', 'it_use_avg', 'it_use_cnt', 'it_update_time', 'it_real_price', 'it_hit')) ){
    $sort='';
}


if(is_file(G5_THIS_SHOP_PATH.'/list.php')) {
	include_once(G5_THIS_SHOP_PATH.'/list.php');
	return;
} else if(is_file(G5_THEME_SHOP_PATH.'/list.php')) {
	include_once(G5_THEME_SHOP_PATH.'/list.php');
	return;
}
if (G5_IS_MOBILE) {
...
<div id="sct">
    ...
    if (file_exists($skin_file)) {
		...
		$ca['ca_list_mod'] = !$ca_id || $ca_id == 'all' ? 2 : $ca['ca_list_mod'];
		$ca['ca_list_row'] = !$ca_id || $ca_id == 'all' ? 10 : $ca['ca_list_row'];
		$ca['ca_img_width'] = !$ca_id || $ca_id == 'all' ? 230 : $ca['ca_img_width'];
		$ca['ca_img_height'] = !$ca_id || $ca_id == 'all' ? 230 : $ca['ca_img_height'];
