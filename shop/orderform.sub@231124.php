...
<div id="sod_frm" class="sod_frm_pc">
	...
	<ul class="sod_list">		
		...
		<li class="sod_li">
			...
			<?php } ?>
			<div class="p20 fs15 fw600 border-bottom"><?=$it_name?></div>
			<div class="li_op_wr">
				<div class="head">
					<?php echo $image; ?>
	...
	<div class="p20 border-top:10">
		<dl class="list_dl">
			<dt>주문총액</dt>
			<dd><?=number_format($tot_sell_price);?> 원</dd>
			<dt>쿠폰</dt>
			<dd><span id="ct_tot_coupon">0</span> 원</dd>
			<dt>배송비</dt>
			<dd><?=number_format($send_cost)?> 원</dd>
			<dt class="total">총 결제금액</dt>
			<?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
			<dd class="total"><span id="ct_tot_price"><?=number_format($tot_price)?></span> 원</dd>
		</dl>
	</div>
    ...
    <div class="sod_">
        ...
        <!-- 주문하시는 분 입력 시작 { -->		
        <section id="sod_frm_orderer" class="border-top:10 p20">
			...
			<ul class="formContainer gap20">
				<li>
					...
					<input type="text" name="od_name" value="<?php echo isset($member['mb_name']) ? get_text($member['mb_name']) : ''; ?>" id="od_name" required class="frm_input required w-full" maxlength="20">
				</li>
				<?php if (!$is_member) { // 비회원이면 ?>
				<li>
					<div class="label">비밀번호</div>
					<input type="password" name="od_pwd" id="od_pwd" required class="frm_input required w-full" maxlength="20" placeholder="영,숫자 3~20자 (주문서 조회시 필요)">
				</li>
				<?php } ?>
				<li>
					<div class="label">전화번호</div>
					<input type="text" name="od_tel" value="<?php echo get_text($member['mb_tel']); ?>" id="od_tel" required class="frm_input required w-full" maxlength="20">
				</li>
				<li>
					<div class="label">연락처</div>
					<input type="text" name="od_hp" value="<?php echo get_text($member['mb_hp']); ?>" id="od_hp" required class="frm_input required w-full" maxlength="20">
					<input type="hidden" name="od_tel" value="<?php echo get_text($member['mb_tel']); ?>" id="od_tel">
				</li>
				<li>
					<div class="label">주소</div>
					<div class="adress flex flex-middle gap8">
						<input type="text" name="od_zip" value="<?php echo $member['mb_zip1'].$member['mb_zip2']; ?>" id="od_zip" required class="frm_input required flex1" size="8" maxlength="6" placeholder="우편번호">
						<button type="button" class="_btn/gray" onclick="win_zip('forderform', 'od_zip', 'od_addr1', 'od_addr2', 'od_addr3', 'od_addr_jibeon');">주소 검색</button>
					</div>
					<p class="mt10">
						<input type="text" name="od_addr1" value="<?php echo get_text($member['mb_addr1']) ?>" id="od_addr1" required class="frm_input frm_address required w-full" size="60" placeholder="기본주소">
					</p>
					<p class="mt10">
						<input type="text" name="od_addr2" value="<?php echo get_text($member['mb_addr2']) ?>" id="od_addr2" class="frm_input frm_address w-full" size="60" placeholder="상세주소">
					</p>
					<p class="mt10">
						<input type="text" name="od_addr3" value="<?php echo get_text($member['mb_addr3']) ?>" id="od_addr3" class="frm_input frm_address w-full" size="60" readonly="readonly" placeholder="참고항목">
						<input type="hidden" name="od_addr_jibeon" value="<?php echo get_text($member['mb_addr_jibeon']); ?>">
					</p>
				</li>
				<li>
					<div class="label">이메일</div>
					<input type="text" name="od_email" value="<?php echo $member['mb_email']; ?>" id="od_email" required class="frm_input required w-full" size="35" maxlength="100">
				</li>
        ...
        <!-- 받으시는 분 입력 시작 { -->
        <section id="sod_frm_taker" class="border-top:10 p20">
            <h2 class="fs16 fw600 mb25">받으시는 분</h2>
			...
			<ul class="formContainer gap20">
                ...
                if($is_member) {
                    ...
                    // 주문자와 동일
                    $addr_list .= '<label class="radio-label"><input type="radio" name="ad_sel_addr" value="same" id="ad_sel_addr_same"><span></span>주문자와 동일</label>'.PHP_EOL;
                ...
                ?>
				<li>
					<div class="label pb15 border-bottom">배송지</div>
					<div class="order_choice_place pt10 flex column gap20">
						<?php echo $addr_list; ?>
					</div>
				</li>
				<?php if($is_member) { ?>
				<li>
					...
					<input type="text" name="ad_subject" id="ad_subject" class="frm_input w-full" maxlength="20" placeholder="배송지명 입력">
					<p class="mt10"><label class="checkbox-label"><input type="checkbox" name="ad_default" id="ad_default" value="1"><span></span>기본배송지로 설정</label></p>
				</li>
				<?php } ?>
				<li>
					<div class="label">이름</div>
				...
				<!--<li>
					<div class="label">전화번호</div>
					<input type="text" name="od_b_tel" id="od_b_tel" required class="frm_input required w-full" maxlength="20" placeholder="전화번호">
				</li>-->
				<li>
					<div class="label">연락처</div>
					<input type="text" name="od_b_hp" id="od_b_hp" required class="frm_input w-full" required maxlength="20" placeholder="휴대폰번호">
				...
				<li>
					...
					<p class="mt10">
						<input type="text" name="od_b_addr3" id="od_b_addr3" readonly="readonly" class="frm_input frm_address w-full" size="60" placeholder="참고항목">
				...
				<li>
					...
					<div class="flex column gap10">
						<select id="od_memo_sel" class="selectpicker relative w-full" data-placeholder="23">
	...
	<div class="border-top:10 p20">
		<div class="fs14 fw600 pb15 border-bottom">결제 정보</div>
		<dl class="list_dl">
			<dt>주문할인 쿠폰</dt>
			<dd class="inline-flex flex-middle gap5"><button type="button" id="od_coupon_btn" class="_btn/small/rd4">쿠폰적용</button></dd>
			<dt>주문할인금액</dt>
			<dd>
				<strong id="od_cp_price">0</strong> 원
				<input type="hidden" name="od_cp_id" value="">
			</dd>
			<dt style="padding-bottom:2em">추가 배송비<p class="absolute color-gray fs12 fw400 mt40">*지역에 따라 추가되는 도선료등의 배송비입니다.</p></dt>
			<dd style="padding-bottom:2em"><strong id="od_send_cost2">0</strong> 원</dd>
			<dt>총 주문금액</dt>
			<?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
			<dd><strong class="print_price"><?php echo number_format($tot_price); ?></strong> 원</dd>
		</dl>
	</div>
	...
	if ($is_kakaopay_use || $default['de_bank_use'] || $default['de_vbank_use'] || $default['de_iche_use'] || $default['de_card_use'] || $default['de_hp_use'] || $default['de_easy_pay_use'] || $default['de_inicis_lpay_use'] || $default['de_inicis_kakaopay_use']) {
		//echo '<fieldset id="sod_frm_paysel" class="bored">';
		//echo '<legend>결제방법 선택</legend>';
		echo '<fieldset id="sod_frm_paysel">';
		
		echo '<div class="border-top:10 p20">';
		echo '<div class="fs14 fw600 pb15 border-bottom">결제 수단</div>';
		echo '<div class="paysel_set py20">';		
	}
	
	// 신용카드 사용
	if ($default['de_card_use']) {
		$multi_settle++;
		echo '<label class="radio-label"><input type="radio" id="od_settle_card" name="od_settle_case" value="신용카드" '.$checked.'><span></span>신용카드</label>'.PHP_EOL;
		$checked = '';
	}
	...
	if ($default['de_iche_use']) {
		$multi_settle++;
		echo '<label class="radio-label"><input type="radio" id="od_settle_iche" name="od_settle_case" value="계좌이체" '.$checked.'><span></span>'.$escrow_title.'계좌이체</label>'.PHP_EOL;
		$checked = '';
	}
	...
	if ($default['de_bank_use']) {
		$multi_settle++;
		echo '<label class="radio-label"><input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" '.$checked.'><span></span>무통장입금</label>'.PHP_EOL;
		$checked = '';
	}
	...
	if($is_kakaopay_use) {
		$multi_settle++;
		echo '<label class="radio-label"><input type="radio" id="od_settle_kakaopay" name="od_settle_case" value="KAKAOPAY" '.$checked.'><span></span>KAKAOPAY</label>'.PHP_EOL;
		$checked = '';
	}
	...
	if ($default['de_vbank_use']) {
		$multi_settle++;
		echo '<label class="radio-label"><input type="radio" id="od_settle_vbank" name="od_settle_case" value="가상계좌" '.$checked.'><span></span>'.$escrow_title.'가상계좌</label>'.PHP_EOL;
		$checked = '';
	}
	...
	if ($default['de_hp_use']) {
		$multi_settle++;
		echo '<label class="radio-label"><input type="radio" id="od_settle_hp" name="od_settle_case" value="휴대폰" '.$checked.'><span></span>휴대폰</label>'.PHP_EOL;
		$checked = '';
	}
	...
	if($default['de_easy_pay_use']) {
		...
		if($default['de_pg_service'] === 'kcp' && isset($default['de_easy_pay_services']) && $default['de_easy_pay_services']){
			...
			if( in_array('nhnkcp_payco', $de_easy_pay_service_array) ){
				$easypay_prints['nhnkcp_payco'] = '<label class="radio-label"><input type="radio" id="od_settle_nhnkcp_payco" name="od_settle_case" data-pay="payco" value="간편결제"><span></span>PAYCO</label>';
			}
			if( in_array('nhnkcp_naverpay', $de_easy_pay_service_array) ){
				$easypay_prints['nhnkcp_naverpay'] = '<label class="radio-label"><input type="radio" id="od_settle_nhnkcp_naverpay" name="od_settle_case" data-pay="naverpay" value="간편결제" ><span></span>네이버페이</label>';
			}
			if( in_array('nhnkcp_kakaopay', $de_easy_pay_service_array) ){
				$easypay_prints['nhnkcp_kakaopay'] = '<label class="radio-label"><input type="radio" id="od_settle_nhnkcp_kakaopay" name="od_settle_case" data-pay="kakaopay" value="간편결제" ><span></span>카카오페이</label>';
			}
		} else {
			$easypay_prints[strtolower($pg_easy_pay_name)] = '<label class="radio-label"><input type="radio" id="od_settle_easy_pay" name="od_settle_case" value="간편결제"><span></span>'.$pg_easy_pay_name.'</label>';
		}
	...
	if($default['de_inicis_lpay_use']) {
		echo '<label class="radio-label"><input type="radio" id="od_settle_inicislpay" data-case="lpay" name="od_settle_case" value="lpay" '.$checked.'><span></span>L.pay</label>'.PHP_EOL;
		$checked = '';
	}

	//이니시스 카카오페이 
	if(isset($default['de_inicis_kakaopay_use']) && $default['de_inicis_kakaopay_use']) {
		echo '<label class="radio-label"><input type="radio" id="od_settle_inicis_kakaopay" data-case="inicis_kakaopay" name="od_settle_case" value="inicis_kakaopay" '.$checked.' title="KG 이니시스 카카오페이"><span></span>KG 이니시스 카카오페이<em></em></label>'.PHP_EOL;
		$checked = '';
	}
	...
	?>
		...
		<dl class="list_dl">
			...
			<dd style="padding-bottom:2em"><strong id="od_send_cost2">0</strong> 원</dd>
			<dt>총 주문금액</dt>
			<dd id="od_tot_price"><strong class="print_price"><?php echo number_format($tot_price); ?></strong> 원</dd>
		</dl>
	...
	<div class="border-top:10 p20">
		<div class="fs14 fw600 pb15 border-bottom">포인트</div>
		<dl class="list_dl">
			<dt class="w-full">사용포인트 (<?php echo $point_unit; ?>점단위)</dt>
			<dd class="w-full flex flex-middle gap10" style="margin-top:-18px;">
				<input type="hidden" name="max_temp_point" value="<?php echo $temp_point; ?>">
				<input type="text" name="od_temp_point" value="0" id="od_temp_point" class="flex1 tright" size="7"> 점
			</dd>
			<dt>보유포인트</dt>
			<dd><?php echo display_point($member['mb_point']); ?></dd>
			<dt>최대 사용 가능 포인트</dt>
			<dd id="use_max_point"><?php echo display_point($temp_point); ?></dd>
		</dl>
		...
		if ($default['de_bank_use']) {
			...
			echo '<div id="settle_bank" class="p20 border-1 rd5 mt10" style="display:none">';
...
$(function() {
    ...
    $(document).on("click", ".od_cp_apply", function() {
        ...
        if(!$("#od_coupon_cancel").length)
            $("#od_coupon_btn").after("<button type=\"button\" id=\"od_coupon_cancel\" class=\"cp_cancel _btn:small\">취소</button>");
    });
