...
echo "aa : ".$sw_direct."<br>";
	...
	$post_it_ids = array($_POST['it_id']);

echo $_POST['it_id']."  //  ".$it_id." // ".$post_it_ids." // ".count($post_it_ids)."<br>";
print_r($post_it_ids);
...
echo "3<br>";
echo "it_buy_min_qty : ".$it['it_buy_min_qty']."<br>";
echo "it_buy_max_qty : ".$it['it_buy_max_qty']."<br>";
        // 최소, 최대 수량 체크
...
echo "comma_sql : ". $sql."<br>";
        ...
        for($k=0; $k<$opt_count; $k++) {
			...
			$sql .= $comma."( '$tmp_cart_id', '{$member['mb_id']}', '{$it['it_id']}', '".addslashes($it['it_name'])."', '{$it['it_sc_type']}', '{$it['it_sc_method']}', '{$it['it_sc_price']}', '{$it['it_sc_minimum']}', '{$it['it_sc_qty']}', '쇼핑', '{$it_price}', '$point', '0', '0', '$io_value', '$ct_qty', '{$it['it_notax']}', '$io_id', '$io_type', '$io_price', '".G5_TIME_YMDHIS."', '$remote_addr', '$ct_send_cost', '$sw_direct', '$ct_select', '$ct_select_time' )"; //인태 - $it_price
            $comma = ' , ';
            $ct_count++;
