...
$cart_all = (isset($_REQUEST['cart_all']) && $_REQUEST['cart_all']) ? 1 : 0;
...
if($_SERVER["REMOTE_ADDR"] == "121.161.30.109"){
	$a = get_session('ss_cart_direct');
	$b = get_session('ss_cart_id');
	//alert($tmp_cart_id." / ".$sw_direct." / ".$a." / ".$b);
}
...
{
	if($cart_all == 1){ //장바구니에 있을경우
		//$tmp_cart_id = get_session('ss_cart_direct');
		$tmp_cart_id = get_session('ss_cart_id');

		//동일상품있을경우 삭제하고 다시담는다.

		$it_id_cart = $post_it_ids[0];

		sql_query(" delete from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id' and it_id = '$it_id_cart' ", false);
		
		

		$sql = " update {$g5['g5_shop_cart_table']} set  ct_direct = '1', ct_select = '1' where od_id = '$tmp_cart_id' ";
		sql_query($sql);

		// 장바구니에 Insert
        $comma = '';
        $sql = " INSERT INTO {$g5['g5_shop_cart_table']}
                        ( od_id, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_status, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_direct, ct_select, ct_select_time, ct_origin_price, ct_time_price)
                    VALUES ";
        ...
        for($k=0; $k<$opt_count; $k++) {
			...
			if($it['it_grade'] == 1){
				$it_price = get_grade_price($it_price); //준섭 등급별금액
			}
            ...
            if($io_type) {
				...
				if((int)$it_price + (int)$io_price < 0)
                    alert('구매금액이 음수인 상품은 구매할 수 없습니다.'); //인태 - $it_price
            }
			...
			*/
            ...
            if($config['cf_use_point']) {
                if($io_type == 0) {
					$point = get_item_point1($it, $io_id,$it_price,$ct_qty);
                    //$point = get_item_point($it, $io_id);
                } else {
					$point = get_item_point1($it, $io_id,'A',$ct_qty);
			...
			$sql .= $comma."( '$tmp_cart_id', '{$member['mb_id']}', '{$it['it_id']}', '".addslashes($it['it_name'])."', '{$it['it_sc_type']}', '{$it['it_sc_method']}', '{$it['it_sc_price']}', '{$it['it_sc_minimum']}', '{$it['it_sc_qty']}', '쇼핑', '{$it_price}', '$point', '0', '0', '$io_value', '$ct_qty', '{$it['it_notax']}', '$io_id', '$io_type', '$io_price', '".G5_TIME_YMDHIS."', '$remote_addr', '$ct_send_cost', '$sw_direct', '$ct_select', '$ct_select_time', '$ct_origin_price', $ct_time_price)"; //인태 - $it_price
            $comma = ' , ';
            $ct_count++;
    ...
    for($i=0; $i<$count; $i++) {
		...
		if($cart_all == 0){
			if($i == 0 && $sw_direct){		
				...
				$itsq = sql_fetch(" select * from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id' and it_id = '$it_id' ", false);
				if($itsq['od_id']){
					sql_query(" delete from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id'  and it_id = '$it_id' ", false);
				}
			}
		}
        // 최소, 최대 수량 체크
...
if ($sw_direct)
{
    if ($is_member)
    {
    	goto_url(G5_SHOP_URL."/orderform.php?sw_direct=$sw_direct&cart_all=$cart_all");
    }
    else
    {
    	goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/orderform.php?sw_direct=$sw_direct&cart_all=$cart_all"));
    }
}
else
{
