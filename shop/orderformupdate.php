...
//인태
if(file_exists(G5_THIS_PATH.'/shop/orderformupdate.php')) {
	//require_once(G5_THIS_PATH.'/shop/orderformupdate.php');
	//return;
}
...
$chk1 = (int)($row['od_price'] - $tot_cp_price);
...
$exists_sql = "select * from {$g5['g5_shop_order_table']} where od_id = '$od_id'";
$exists_order = sql_fetch($exists_sql);

// 원주문 데이터를 직렬화 하여 저장한다.
$exists_sql = "update {$g5['g5_shop_order_table']} set od_data = '".base64_encode(serialize($exists_order))."' where od_id = '$od_id'";
sql_query($exists_sql, false);
...
if(!$result) {
	...
	$sql1 = "INSERT INTO `g5_shop_order_post`
			SELECT * FROM `g5_shop_order`
			WHERE od_id = '".$od_id."' ";
	sql_query($sql1);

	$sql2 = "INSERT INTO `g5_shop_order_post_cart`
...
if($is_member) {
    ...
    for($i=0; $i<$it_cp_cnt; $i++) {
        ...
        if(trim($cid)) {
			...
			$sql1 = " select cz_id,cp_subject from {$g5['g5_shop_coupon_table']} where cp_id = '".$cid."' ";
			$cp1 = sql_fetch($sql1);

            $sql = " insert into {$g5['g5_shop_coupon_log_table']}
                        set cp_id       = '$cid',
						    cz_id		= '".$cp1['cz_id']."',
							cz_subject  = '".$cp1['cp_subject']."',
                            mb_id       = '{$member['mb_id']}',
                            it_id       = '$cp_it_id',
                            od_id       = '$od_id',
                            cp_price    = '$cp_prc',
    ...
    if(isset($_POST['od_cp_id']) && $_POST['od_cp_id']) {
		...
		$sql1 = " select cz_id,cp_subject from {$g5['g5_shop_coupon_table']} where cp_id = '".$_POST['od_cp_id']."' ";
		$cp1 = sql_fetch($sql1);
        $sql = " insert into {$g5['g5_shop_coupon_log_table']}
    ...
    if(isset($_POST['sc_cp_id']) && $_POST['sc_cp_id']) {
		...
		$sql1 = " select cz_id,cp_subject from {$g5['g5_shop_coupon_table']} where cp_id = '".$_POST['sc_cp_id']."' ";
		$cp1 = sql_fetch($sql1);
...
//if($_SERVER["REMOTE_ADDR"] == "121.161.30.109"){
	...
	for($k=0; $ct=sql_fetch_array($cresult); $k++) {
		...
		{	
			...
			if ($ct_status == '주문' || $ct_status == '입금' || $ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절')
			{
				$stock_use = 0;
				// 재고에 다시 더한다.
				if($ct['io_id']) {
...
$sms_content = $default['de_sms_cont3'];
$sms_content4 = $default['de_sms_cont4'];

if ($is_member){
	$recv_numbers = $od_hp;
	$recv_number = preg_replace("/[^0-9]/", "", $recv_numbers);
}else{
	$recv_numbers = $od_b_hp;
	$recv_number = preg_replace("/[^0-9]/", "", $recv_numbers);
}
...
$sms_content = str_replace("{주문금액}", number_format($i_price), $sms_content);
...
$sms_content = str_replace("{계좌정보}", $od_bank_account, $sms_content);


$sms_content4 = str_replace("{이름}", $od_name, $sms_content4);
$sms_content4 = str_replace("{회원아이디}", $member['mb_id'], $sms_content4);
$sms_content4 = str_replace("{회사명}", $default['de_admin_company_name'], $sms_content4);
$sms_content4 = str_replace("{주문번호}", $od_id, $sms_content4);
$sms_content4 = str_replace("{주문금액}", number_format($i_price), $sms_content4);
//$sms_content4 = str_replace("{주문일자}", G5_TIME_YMDHIS, $sms_content4);
...
if($config['cf_sms_use'] == "aligo"){
	...
	$send_numbers = $config['cf_aligo_sender'];
	$send_number = preg_replace("/[^0-9]/", "", $send_numbers);

	//if($is_sms_send) {
		
		// 무통장 입금 때 고객에게 계좌정보 보냄
		if($od_settle_case == '무통장' && $default['de_sms_use3'] && $od_misu > 0) {
			//$sms_content = $od_name."님의 입금계좌입니다.\n금액:".number_format($od_misu)."원\n계좌:".$od_bank_account."\n".$default['de_admin_company_name'];

			//무통장
			//order_email_call('무통장완료', $od_email, $member['mb_id'], $od_id, $od_name, G5_TIME_YMDHIS,  $default['de_admin_company_name'], $od_bank_account, $sms_content);
			
			$sql = " insert into `g5_shop_order_email`
						set od_type = '무통장완료',
							od_email = '".$od_email."',
							od_id = '".$od_id."',
							od_name = '".$od_name."',
			...
			//주문완료
			//order_email_call('결제완료', $od_email, $member['mb_id'], $od_id, $od_name, G5_TIME_YMDHIS,  $default['de_admin_company_name'], $od_bank_account, $sms_content4);
			...
			//담당자발송
			aligo_sms_call("무통장 주문이 들어왔습니다.", $config['cf_manager_order_hp'], $send_number, "", "", "");
			//order_email_call('무통장완료', $config['cf_manager_order_email'], $member['mb_id'], $od_id, $od_name, G5_TIME_YMDHIS,  $default['de_admin_company_name'], $od_bank_account, $sms_content);


			//알림톡발송 무통장입금정보
			//$tpl_code, $recevier, $name="", $od_id="", $od_price="", $product="", $md_id="", $account="", $company="", $invoice=""
			$kresult1 = kakao_alim("TS_0376", $recv_number, $od_name, $od_id, $i_price, "", "", $od_bank_account, "", "");
			if($kresult1 != 'Y'){
				//내용,받는사람번호,보낸사람번호,SMS or LMS,제목,구분
				//고객발송
				aligo_sms_call($sms_content, $recv_number, $send_number, "", "", "");
			}

			//주문완료
			$kresult2 = kakao_alim("TS_0377", $recv_number, $od_name, $od_id, $i_price, "", "", "", "", "");
			if($kresult2 != 'Y'){
				//내용,받는사람번호,보낸사람번호,SMS or LMS,제목,구분
				aligo_sms_call($sms_content4, $recv_number, $send_number, "", "", "");
			}
			...
			//담당자발송
			aligo_sms_call("결제 완료된 주문이 들어왔습니다.", $config['cf_manager_order_hp'], $send_number, "", "", "");
			//order_email_call('결제완료', $config['cf_manager_order_email'], $member['mb_id'], $od_id, $od_name, G5_TIME_YMDHIS,  $default['de_admin_company_name'], $od_bank_account, $sms_content4);
			...
			$kresult2 = kakao_alim("TS_0377", $recv_number, $od_name, $od_id, $i_price, "", "", "", "", "");
			if($kresult2 != 'Y'){
				//내용,받는사람번호,보낸사람번호,SMS or LMS,제목,구분//고객발송
				aligo_sms_call($sms_content4, $recv_number, $send_number, "", "", "");
			}
		}
	//}
	...
	$send_numbers = $config['cf_naver_sender'];
	$send_number = preg_replace("/[^0-9]/", "", $send_numbers);
	
	//네이버는 SMS, LMS 문구가 필수
	//메세지,보내는사람,받는사람,SMS/LMS,제목,구분
	naver_sms_call($sms_content, $recv_number, $send_number, "LMS", "", "");
