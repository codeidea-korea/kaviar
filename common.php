...
if (isset($_REQUEST['tag']))  {
    $tag = get_search_string(trim($_REQUEST['tag']));
    if ($tag || $tag === '0')
        $qstr .= '&amp;tag=' . urlencode(cut_str($tag, 30, ''));
} else {
    $tag = '';
}
...
//member 경로 수정
$member_skin_path = G5_BBS_PATH.'/my/member';
$member_skin_url = G5_BBS_URL.'/my/member';

//==============================================================================
$config_apppush = sql_fetch(" select * from `g5_config_apppush` ");
