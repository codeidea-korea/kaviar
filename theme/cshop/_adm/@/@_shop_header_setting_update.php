...
$count = isset($_POST['shopmenu']) ? count($_POST['shopmenu']) : 0;

for ($i=0; $i<$count; $i++) {
    $_POST = array_map_deep('trim', $_POST);	
	
	$shopmenu_order = $_POST['shopmenu_order'][$i] ? $_POST['shopmenu_order'][$i] : '0';
	$shopmenu = $_POST['shopmenu'][$i];
	$shopmenu_name = $_POST['shopmenu'][$i] ? '' : preg_replace('/\r\n|\r|\n/','',$_POST['shopmenu_name'][$i]);
	$shopmenu_link = $_POST['shopmenu_link'][$i] ? $_POST['shopmenu_link'][$i] : '#';
    ...
    $sql = " insert into {$g5['g5_shop_top_menu_table']} 
                set shopmenu_order		= '{$shopmenu_order}',
					 shopmenu					= '{$shopmenu}',
					 shopmenu_name		= '{$shopmenu_name}',
					 shopmenu_link			= '{$shopmenu_link}' ";
    sql_query($sql);
}
