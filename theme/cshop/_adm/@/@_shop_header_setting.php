		...
		<div class="form-list">
			<div class="formCon column gap0">
				<div class="flex flex-middle gap20">
					<b>상단 메뉴관리</b>
					<button type="button" onclick="return add_menu();" class="_btn/mini/gray2/rd5 fw500">메뉴추가</button>
					<?php if(!$itemtype) echo '<span class="help-block color-red ml10">※ 선택가능 메뉴가 없다면 <b>[상품유형]</b>을 먼저 설정해 주세요.</span>'; ?>
				</div>
				<ul id="shop_top_menu">
					<?php
					include_once($_adm_inc_path.'/shop_top_menu.lib.php');
...
function menuChoice(el) {
	$(el).on("change", function(){
		var val = $(this).find("option:selected").val(),
			target = $(this).closest('li').find('.menuform');
		if(val) {
			target.hide().removeClass('flex');
		} else {
			target.show().addClass('flex');						
		}
	});
}
...
function add_menu() {	
	...
	$.post("<?=$_adm_update_url?>/shop_top_menu_form.php",{count:count}, function(data) {
