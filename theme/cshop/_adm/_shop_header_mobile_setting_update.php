...
if($del_shop_logo_mobile_w) @unlink(G5_DATA_PATH.'/logo/del_shop_logo_mobile_w.png');
...
if(is_uploaded_file($_FILES['shop_logo_mobile_w']['tmp_name'])) {
	if (!preg_match($image_regex, $_FILES['shop_logo_mobile_w']['name']))
		alert($_FILES['shop_logo']['name'] . '은(는) 이미지 파일이 아닙니다.');
	if (preg_match($image_regex, $_FILES['shop_logo_mobile_w']['name'])) {
		$dest_path = G5_DATA_PATH.'/logo/shop_logo_mobile_w.png';
		move_uploaded_file($_FILES['shop_logo_mobile_w']['tmp_name'], $dest_path);
		chmod($dest_path, G5_FILE_PERMISSION);
	}
}
...
if($del_shop_hdIcon_brand) @unlink($shop_icon_path.'/shop_hdIcon_brand.svg');
...
if(is_uploaded_file($_FILES['shop_hdIcon_brand']['tmp_name'])) {
	if (!preg_match($icon_regex, $_FILES['shop_hdIcon_brand']['name']))
		alert($_FILES['shop_hdIcon_brand']['name'] . '은(는) svg 파일이 아닙니다.');
	if (preg_match($icon_regex, $_FILES['shop_hdIcon_brand']['name'])) {
		$dest_path = $shop_icon_path.'/shop_hdIcon_brand.svg';
		@move_uploaded_file($_FILES['shop_hdIcon_brand']['tmp_name'], $dest_path);
		@chmod($dest_path, G5_FILE_PERMISSION);
	}
}
