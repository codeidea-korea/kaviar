...
<div class="p20" style="--form-height:48px;">
	...
	<ul class="formContainer gap30 mt10">
		...
		<?php if($config['cf_use_membercode']) {
			if($w=='') {
				echo '<li>';
					echo '<div class="label"><span class="color-black">기업코드</span></div>';
					echo '<input type="text" name="mb_code" value="'.get_text($member['mb_code']).'" id="mb_code" class="frm_input full_input w-full '.($config['cf_req_membercode']?"required":"").'" maxlength="20" placeholder="기업코드'.($config['cf_req_membercode']?'(필수)':'').'">';
				echo '</li>';
			} else {
				echo '<input type="hidden" name="mb_code" value="'.get_text($member['mb_code']).'" id="mb_code">';
			}
		} ?>
...
function fregisterform_submit(f)
{
	...
	<?php if($w == '' && $config['cf_req_membercode'] && $config['cf_use_membercode']) {
	?>
	// 기업코드
	if (f.w.value=='') {
		if (f.mb_code.value.length < 1) {
			alert('가입코드를 입력하십시오.');
			f.mb_code.focus();
			return false;
		}
