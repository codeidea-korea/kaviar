...
if($cf_default_style[0]&&file_exists(G5_CSS_URL.'/font/'.$cf_default_style[0].'.css')) echo '<link rel="stylesheet" href="'.get_url(G5_CSS_URL.'/font/'.$cf_default_style[0].'.css').'">'.PHP_EOL; //사이트 기본폰트
...
add_stylesheet('<link rel="stylesheet" href="'.get_url(G5_CSS_URL.'/_shop_common'.(G5_IS_MOBILE?'_mobile':'').'.css').'">', 3); //쇼핑몰 공통
...
<body<?php echo isset($g5['body_script']) ? $g5['body_script'] : ''; ?><?=$cf_default_style[0]?' data-font-family="'.$cf_default_style[0].'"':''?><?=$style_mainColor?' style="'.$style_mainColor.'"':''?> data-shop-layout="<?=$default['shop_layout']?$default['shop_layout']:'outside-right'?>">
<?php
