...
define('_EVENT_', true); //인태 - 추가
include_once('./_common.php');
if (!defined('_SHOP_COMMON_')) exit; // 모바일 페이지로 직접 접근하는 것을 막음

$ev_id = isset($_GET['ev_id']) ? (int) $_GET['ev_id'] : '';

if(!$ev_id && strpos($_SERVER[ "REQUEST_URI" ], 'shop/event/') !== false) {
	$ev_id = basename( $_SERVER[ "PHP_SELF" ] );
}

if(!$ev_id) {
	require_once(G5_MSHOP_PATH.'/_event_main.php');
	return;
}
...
echo '<div id="_event_list">';

	if($is_admin) echo '<a href="'.G5_ADMIN_URL.'/shop_admin/itemeventform.php?w=u&amp;ev_id='.$ev['ev_id'].'" class="btnSetting _blank" target="_blank" data-area="#_event_list">이벤트 관리</a>';

	// 상단 HTML
	if($ev['ev_head_html']) echo '<div id="sev_hhtml" class="_hhtml">'.conv_content($ev['ev_head_html'], 1).'</div>';
	...
	// 하단 HTML
	if($ev['ev_tail_html']) echo '<div id="sev_thtml" class="_thtml">'.conv_content($ev['ev_tail_html'], 1).'</div>';
