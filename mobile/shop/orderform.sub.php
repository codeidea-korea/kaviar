...
<div id="sod_frm" class="sod_frm_pc">
	...
	<ul class="sod_list">
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            $it_name = '<b class="li_name">' . stripslashes($row['it_name']) . '</b>';
			...
			*/
			  ...
			  or mb_grade like '%{$member['mb_grade']}%')
                ...
                if($cp_count) {
                    $cp_button = '<div class="flex flex-middle gap5"><button type="button" class="cp_btn _btn/sm/line/mainColor/transparent">쿠폰적용</button></div>';
                    $it_cp_count++;
                }
            }
		...
		<li class="sod_li">
			...
			<?php } ?>
			<div class="p20 fs15 fw600 border-bottom"><?=stripslashes($row['it_name'])?></div>
			<div class="li_op_wr">
				...
				<div class="li_prqty">
					<div class="divide">
						<span class="label"><?=$it_type?'예약가':'판매가'?></span><span class="val"><?php echo number_format($row['ct_price']); ?></span>
						<?php if(!$it_type) { ?>
					...
					<div class="divide">
						...
						<span class="label">적립포인트</span><span class="val"><?php echo number_format($point); ?></span>
					</div>
				</div>
				<div class="li_total">
					<span class="label"><?=$it_type?'예약금액':'주문금액'?></span><span class="val"><?php echo number_format($sell_price); ?> 원</span>
				</div>
			</div>
		</li>
        <?php
	...
	<!-- 주문상품 합계 시작 { -->
	<div id="sod_bsk_tot" class="p20 border-top">
		<ul class="_list_info">
			<li>
				<span class="label">주문총액</span>
				<span><?=number_format($tot_sell_price);?> 원</span>
			</li>
			<li>
				<span class="label">쿠폰할인</span>
				<span><span id="ct_tot_coupon">0</span> 원</span>
			</li>
			<li>
				<span class="label">배송비</span>
				<span><?=number_format($send_cost)?> 원</span>
			</li>
			<li>
				<span class="label">총계</span>
				<?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
				<span class="total"><span id="ct_tot_price"><?=number_format($tot_price)?></span> 원</span>
			</li>
		</ul>
	</div>
	<!-- } 주문상품 합계 끝 -->    
    ...
    <div class="sod_">
        ...
        <!-- 주문하시는 분 입력 시작 { -->
		<?php if(!$it_type) { ?>
        <section id="sod_frm_orderer" class="border-top p20">
            <h2 class="fs16 fw600 mb15">주문하시는 분</h2>			
			<?php
			//주문자 정보
			if ($is_member) {
				echo '<div id="sod_orderer_info">';
					echo '<div class="orderer_name">'.get_text($member['mb_name']).'</div>';
					$mb_addr_info = $member['mb_addr1'] && $member['mb_addr1'] ? $member['mb_addr1'].$member['mb_addr2'].$member['mb_addr3'].$member['mb_jibeon'] : '<span class="color-gray">주소 정보가 없습니다.</span>';
					echo '<div class="orderer_addr">'.$mb_addr_info.'</div>';
					echo '<div class="flex flex-middle gap15">';
						$mb_hp_info = $member['mb_hp'] ? $member['mb_hp'] : '<span class="color-gray">연락처 정보가 없습니다.</span>';
						echo '<div class="orderer_hp">'.$mb_hp_info.'</div>';
						$mb_email_info = $member['mb_email'] ? $member['mb_email'] : '<span class="color-gray">이메일 정보가 없습니다.</span>';
						echo '<div class="orderer_email">'.$mb_email_info.'</div>';
					echo '</div>';
				echo '</div>';
        ...
        <!-- 받으시는 분 입력 시작 { -->
        <section id="sod_frm_taker" class="border-top p20">
            <h2 class="fs16 fw600 mb25"><?=$it_type?'예약자 정보':'받으시는 분'?></h2>			
			<ul class="formContainer gap20">
				...
				if(!$it_type) {
					...
					if($is_member) {
						...
						// 주문자와 동일
						$addr_list .= '<label class="radio-label"><input type="radio" name="ad_sel_addr" value="same" id="ad_sel_addr_same" checked><span></span>주문자와 동일</label>'.PHP_EOL;
						...
						if(isset($row['ad_id']) && $row['ad_id']) {
							...
							$addr_list .= '<label class="radio-label"><input type="radio" name="ad_sel_addr" value="'.get_text($val1).'" id="ad_sel_addr_def"><span></span>기본배송지</label>'.PHP_EOL;
						}
						...
						for($i=0; $row=sql_fetch_array($result); $i++) {
							...
							$addr_list .= '<label class="radio-label"><input type="radio" name="ad_sel_addr" value="'.get_text($val1).'" id="ad_sel_addr_'.($i+1).'"><span></span>'.$val2.'</label>'.PHP_EOL;
						}
						
						$addr_list .= '<div class="flex flex-middle gap15 mt-6">';
						$addr_list .= '<label class="radio-label"><input type="radio" name="ad_sel_addr" value="new" id="od_sel_addr_new"><span></span>신규배송지</label>'.PHP_EOL;
						$addr_list .='<a href="'.G5_SHOP_URL.'/orderaddress.php" id="order_address" class="_btn/small/line/rd4 px8">배송지목록</a>';
						$addr_list .= '</div>';
					} else {
						// 주문자와 동일
						$addr_list .= '<label class="radio-label"><input type="checkbox" name="ad_sel_addr" value="same" id="ad_sel_addr_same"><span></span>주문자와 동일</label>'.PHP_EOL;
					}
					echo '<li>';
						echo '<div class="label pb15 border-bottom">배송지</div>';
						echo '<div class="order_choice_place pt10 flex column gap20">';
							echo $addr_list;
						echo '</div>';
					echo '</li>';
					if($is_member) {
						echo '<li>';
							...
							echo '<input type="text" name="ad_subject" id="ad_subject" class="frm_input w-full transparent" maxlength="20" placeholder="배송지명 입력">';
				...
				<li>
					...
					<input type="text" name="od_b_name" id="od_b_name" required class="frm_input required w-full transparent" maxlength="20" placeholder="공백없이 한글,영문,숫자만 입력 가능 (한글2자, 영문4자 이상)">
				</li>
				<li>
					<div class="label">연락처</div>
					<input type="text" name="od_b_hp" id="od_b_hp" required class="frm_input w-full transparent required" required maxlength="20" placeholder="휴대폰번호">
				...
				<li>
					...
					<div class="adress flex flex-middle gap8">
						<input type="text" name="od_b_zip" id="od_b_zip" required class="frm_input required flex1 transparent" size="8" maxlength="6" placeholder="우편번호">
					...
					<p class="mt10">
						<input type="text" name="od_b_addr1" id="od_b_addr1" required class="frm_input frm_address required w-full transparent" size="60" placeholder="기본주소">
					</p>
					<p class="mt10">
						<input type="text" name="od_b_addr2" id="od_b_addr2" class="frm_input frm_address w-full transparent" size="60" placeholder="상세주소">
					</p>
				...
				<li>
					...
					<div class="flex flex-middle gap10">
						<input type="text" name="od_hope_date" value="" id="od_hope_date" required class="frm_input required w-70 transparent" size="11" maxlength="10" readonly="readonly"> 이후로 배송 바랍니다.
					</div>
				</li>
				<?php } ?>
				<li>
					...
					<div class="flex column gap10">
						<select id="od_memo_sel" class="selectpicker relative w-full">
	...
	<section id="sod_frm_pay">
		...
		?>			
		<!-- 결제정보 -->		
		<div id="pay_info" class="border-top p20">
			<div class="fs14 fw600 pb15">결제 정보</div>
			<ul class="_list_info mt15">
				<?php if($oc_cnt > 0) { ?>
				<li>
					<span class="label"><?=$it_type?'예약할인':'주문할인'?> 쿠폰</span>
					<button type="button" id="od_coupon_btn" class="_btn/sm/rd4">쿠폰적용</button>
				</li>
				<li>
					<span class="label">주문할인금액</span>
					<span><strong id="od_cp_price">0</strong> 원</span>
				...
				<?php if($sc_cnt > 0) { ?>
				<li>
					<span class="label">배송비할인 쿠폰</span>
					<button type="button" id="sc_coupon_btn" class="_btn/sm/rd4">쿠폰적용</button>
				</li>
				<li>
					<span class="label">배송비할인금액</span>
					<span><strong id="sc_cp_price">0</strong> 원</span>
					<input type="hidden" name="sc_cp_id" value="">
				</li>
				<?php } ?>
				<li>
					<div class="label">추가 배송비<p class="absolute color-gray fs12 fw400 mt40">*지역에 따라 추가되는 도선료등의 배송비입니다.</p></div>
					<div><strong id="od_send_cost2">0</strong> 원</div>
				</li>
				<?php
				} else { //!$it_type
				...
				<li>
					...
					<?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
					<span id="od_tot_price" class="total color-red"><strong class="print_price"><?php echo number_format($tot_price); ?></strong> 원</span>
				</li>
			</ul>
		</div>
		<!-- //결제정보 -->
		...
		<div id="od_pay_sl" class="mt20">
			...
			if ($is_kakaopay_use || $default['de_bank_use'] || $default['de_vbank_use'] || $default['de_iche_use'] || $default['de_card_use'] || $default['de_hp_use'] || $default['de_easy_pay_use'] || $default['de_inicis_lpay_use'] || $default['de_inicis_kakaopay_use']) {
				echo '<fieldset id="sod_frm_paysel">';
					...
					echo '<div class="paysel_set_container border-top p20">';
						echo '<div class="pb15 fs14 fw600">결제 수단</div>';
						echo '<ul class="paysel_set py15">';					
			}
			
			// 신용카드 사용
			if ($default['de_card_use']) {
				$multi_settle++;
				echo '<li><label class="radio-label radio-btn"><input type="radio" id="od_settle_card" name="od_settle_case" value="신용카드" '.$checked.' class="radio-btn"><span>신용카드</span></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if ($default['de_iche_use']) {
				$multi_settle++;
				echo '<li><label class="radio-label radio-btn"><input type="radio" id="od_settle_iche" name="od_settle_case" value="계좌이체" '.$checked.' class="radio-btn"><span>'.$escrow_title.'계좌이체</span></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if ($default['de_bank_use']) {
				$multi_settle++;
				echo '<li><label class="radio-label radio-btn"><input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" '.$checked.' class="radio-btn"><span>무통장입금</span></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if($is_kakaopay_use) {
				$multi_settle++;
				echo '<li><label class="radio-label radio-btn"><input type="radio" id="od_settle_kakaopay" name="od_settle_case" value="KAKAOPAY" '.$checked.' class="radio-btn"><span>KAKAOPAY</span></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if ($default['de_vbank_use']) {
				$multi_settle++;
				echo '<li><label class="radio-label radio-btn"><input type="radio" id="od_settle_vbank" name="od_settle_case" value="가상계좌" '.$checked.' class="radio-btn"><span>'.$escrow_title.'가상계좌</span></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if ($default['de_hp_use'] && $is_admin) {  //$is_admin - 휴대폰결제 오류로 임시로 관리자만 보이도록 수정
				$multi_settle++;
				echo '<li><label class="radio-label radio-btn"><input type="radio" id="od_settle_hp" name="od_settle_case" value="휴대폰" '.$checked.' class="radio-btn"><span>휴대폰</span></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if($default['de_easy_pay_use']) {
				...
				if($default['de_pg_service'] === 'kcp' && isset($default['de_easy_pay_services']) && $default['de_easy_pay_services']){
					...
					if( in_array('nhnkcp_payco', $de_easy_pay_service_array) ){
						$easypay_prints['nhnkcp_payco'] = '<li><label class="radio-label"><input type="radio" id="od_settle_nhnkcp_payco" name="od_settle_case" data-pay="payco" value="간편결제"><span></span>PAYCO</label></li>';
					}
					if( in_array('nhnkcp_naverpay', $de_easy_pay_service_array) ){
						$easypay_prints['nhnkcp_naverpay'] = '<li><label class="radio-label"><input type="radio" id="od_settle_nhnkcp_naverpay" name="od_settle_case" data-pay="naverpay" value="간편결제" ><span></span>네이버페이</label></li>';
					}
					if( in_array('nhnkcp_kakaopay', $de_easy_pay_service_array) ){
						$easypay_prints['nhnkcp_kakaopay'] = '<li><label class="radio-label"><input type="radio" id="od_settle_nhnkcp_kakaopay" name="od_settle_case" data-pay="kakaopay" value="간편결제" ><span></span>카카오페이</label></li>';
					}
				} else {
					$easypay_prints[strtolower($pg_easy_pay_name)] = '<li><label class="radio-label"><input type="radio" id="od_settle_easy_pay" name="od_settle_case" value="간편결제"><span></span>'.$pg_easy_pay_name.'</label></li>';
				}
			...
			if($default['de_inicis_lpay_use']) {
				echo '<li><label class="radio-label"><input type="radio" id="od_settle_inicislpay" data-case="lpay" name="od_settle_case" value="lpay" '.$checked.'><span></span>L.pay</label></li>'.PHP_EOL;
				$checked = '';
			}

			//이니시스 카카오페이 
			if(isset($default['de_inicis_kakaopay_use']) && $default['de_inicis_kakaopay_use']) {
				echo '<li><label class="radio-label"><input type="radio" id="od_settle_inicis_kakaopay" data-case="inicis_kakaopay" name="od_settle_case" value="inicis_kakaopay" '.$checked.' title="KG 이니시스 카카오페이"><span></span>KG 이니시스 카카오페이<em></em></label></li>'.PHP_EOL;
				$checked = '';
			}
			...
			if ($is_member && $config['cf_use_point']) {
				...
				if ($member['mb_point'] >= $default['de_settle_min_point']) {
					...
					echo '<div id="sod_frm_point" class="border-top p20">';
						echo '<div class="fs14 fw600 pb15">포인트</div>';
						echo '<ul class="_list_info">';
							echo '<li class="column">';
								echo '<span class="label">사용포인트 ('.$point_unit.'점단위)</span>';
								echo '<div class="w-full flex flex-middle gap10">';
									echo '<input type="hidden" name="max_temp_point" value="'.$temp_point.'">';
									echo '<input type="text" name="od_temp_point" value="0" id="od_temp_point" class="flex1 tright transparent" size="7"> 점';
								echo '</div>';
							echo '</li>';
							echo '<li>';
								echo '<span class="label">보유포인트</span>';
								echo '<span>'.display_point($member['mb_point']).'</span>';
							echo '</li>';
							echo '<li>';
								echo '<span class="label">최대 사용 가능 포인트</span>';
								echo '<span id="use_max_point">'.display_point($temp_point).'</span>';
							echo '</li>';
						echo '</ul>';
					echo '</div>';
				...
				echo '<div class="px20">';
					echo '<div id="settle_bank" class="p20 py10 border-1 rd5 mt10" style="display:none">';
						echo '<dl class="list_dl">';
							...
							echo '<dt>입금자명</dt>';
							echo '<dd><input type="text" name="od_deposit_name" id="od_deposit_name" size="10" maxlength="20" class="transparent"></dd>';
						echo '</dl>';
					echo '</div>';
				echo '</div>';
			}
