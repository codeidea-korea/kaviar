...
add_stylesheet('<link rel="stylesheet" href="'.get_url(get_social_skin_url().'/style.css').'">', 13);
...
<div class="social_register">
    ...
    <form name="fregisterform" id="fregisterform" action="<?php echo $register_action_url; ?>" onsubmit="return fregisterform_submit(this);" method="POST" autocomplete="off">
		...
		<div class="textbox h-auto fs13 fw500">
			회원가입약관 및 개인정보 수집 및 이용의 내용에 동의하셔야<br>
			회원가입 하실 수 있습니다.
		</div>
		...
		<div class="mt20">
			<label class="checkbox-label fw500"><input type="checkbox" name="agree" value="1" class="chk1"><span></span>회원가입약관 (필수)</label>
		...
		<div class="mt20">
			<label class="checkbox-label fw500"><input type="checkbox" name="agree2" value="1" class="chk1"><span></span>개인정보 수집 및 이용 (필수)</label>
			<div class="textbox h-135 fs13 color-gray2 mt15">
        ...
        <div id="register_form" class="py20 border-top">
            <h2 class="fs15 fw600">개인정보 입력</h2>
            <ul class="formContainer">                
				<?php
				if ($config['cf_cert_use']) {
					echo '<li class="flex flex-middle">';
						if ($config['cf_cert_simple']) {
							echo '<button type="button" id="win_sa_kakao_cert" class="btn_frmline btn win_sa_cert _btn/sm/line" data-type="">간편인증</button>'.PHP_EOL;
						}
						if ($config['cf_cert_hp'])
							echo '<button type="button" id="win_hp_cert" class="btn_frmline btn _btn/sm/line">휴대폰 본인확인</button>' . PHP_EOL;
						if ($config['cf_cert_ipin'])
							echo '<button type="button" id="win_ipin_cert" class="btn_frmline btn _btn/sm/line">아이핀 본인확인</button>' . PHP_EOL;
				...
				if ($req_nick) {
					echo '<li>';
						echo '<div class="fs14 fw600 mb5">닉네임</div>';
						echo '<label for="reg_mb_nick" class="sound_only">닉네임 (필수)</label>';
						echo '<div class="frm_info f212 color-slate-500 mb10">';
							echo '공백없이 한글,영문,숫자만 입력 가능 (한글2자, 영문4자 이상)<br>';
							echo '닉네임을 바꾸시면 앞으로 '.(int)$config['cf_nick_modify'].'일 이내에는 변경 할 수 없습니다.';
						echo '</div>';
						echo '<input type="hidden" name="mb_nick_default" value="'.(isset($user_nick) ? get_text($user_nick) : '').'">';						
						echo '<input type="text" name="mb_nick" value="'.(isset($user_nick) ? get_text($user_nick) : '').'" id="reg_mb_nick" required class="frm_input required nospace w-full" size="10" maxlength="20" placeholder="닉네임 (필수)">';
						echo '<span id="msg_mb_nick"></span>';
					echo '</li>';
                }
				?>
                <li>
					<div class="fs14 fw600 mb5">이메일</div>
        ...
        <div class="btn_confirm flex flex-middle">
            <a href="<?php echo G5_URL ?>" class="btn_cancel _btn/lg/line">취소</a>
            <button type="submit" id="btn_submit" class="btn_submit _btn/lg/black" accesskey="s"><?php echo $w == '' ? '회원가입' : '정보수정'; ?></button>
        </div>
...
<div class="member_connect p15">
    <p class="fs14 tcenter mb15">혹시 기존 회원이신가요?</p>
    <button type="button" class="connect-opener btn-txt _btn/lg/line w-full" data-remodal-target="modal">기존 계정에 연결하기</button>
</div>
