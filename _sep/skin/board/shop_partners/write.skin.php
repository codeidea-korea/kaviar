...
<section id="bo_w" style="<?=$bo_width?>">
    ...
    <div class="wr-wrap" style="--label-width:130px;">
		<?=$wr_guest?>

		<?php if($wr_include) echo '<div class="wr-group">'.$wr_include.'</div>'; ?>
		
		<?php
		echo '<div class="wr-list">'.PHP_EOL;
			echo '<div class="wr-list-label"><label class="label-order">'.$store_label.' 선택</label></div>'.PHP_EOL;
			echo '<div class="wr-list-con">'.PHP_EOL;
				echo '<select name="wr_store_id" value="'.$write['wr_store_id'].'" id="wr_store_id" class="selectpicker">';
					...
					echo option_selected_my('',  $write['wr_store_id'], "", "data-content='연결할 ".$store_label." 선택'");
					for ($i=0; $row=sql_fetch_array($shop_result); $i++) {
						echo option_selected_my($row['store_id'],  $write['wr_store_id'], $row['store_subject'], "data-content='".$row['store_subject']." <span class=\"fw300\">(".get_store_items_count($row['store_id']).")</span>'");
					}
				echo '</select>'.PHP_EOL;
				if($is_admin) echo '<a href="'.G5_ADMIN_URL.'/shop_admin/my/storelist.php" class="_btn/rd5" target="_blank">'.$store_label.' 관리</a>';
			echo '</div>';
		echo '</div>';
		?>

		<?php
		if(!G5_IS_MOBILE) echo '<div class="wr-group">';
		echo $wr_gall_file;			
		echo $wr_video;
		if(!G5_IS_MOBILE) echo '</div>';
		?>
