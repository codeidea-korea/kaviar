...
jQuery(function ($) {
    ...
    mainCart.add_cart = function(frm) {
        ...
        if(count > 0) {
            $sel.each(function(index) {
				...
				item  = $(this).parent().prev("label").text(); //황팀수정 - bootstrap-select로 인해 상대적위치 변경
    ...
    $(document).on("click", ".btn_cart", function(e) {
		...
		$("#cart-layer").remove();
		$('#wrapper').append('<div id="cart-layer"></div>');

        var $this = $(this),
            it_id = $this.data("it_id"),
            $sct_li = $this.closest(".sct_li"),
			$opt = $('body').find("#cart-layer"),
        ...
        $.ajax({
            ...
            success: function(data, textStatus) {
				...
				$("#cart-layer select").selectpicker('refresh'); //인태 추가
        ...
        if(sel_count > 1 && (idx + 1) < sel_count) {
            ...
            $.post(
                ...
                function(data) {
					...
					$frm.find("select.it_option").selectpicker('refresh');
                }
            );
