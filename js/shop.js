...
$(function() {
    ...
    */
    ...
    if(isAndroid) {
		...
		$(document).on("touchend", ".it_option", function() { //황팀 - 수정
            option_add = true;
        });
    } else {
		...
		$(document).on(it_option_events, ".it_option", function(e) { //황팀 - 수정
            option_add = true;
        });
    }
    ...
    $(document).on("change", "select.it_option", function() {
        ...
        if(val == "") {
			...
			$("select.it_option:gt("+idx+")").selectpicker('refresh'); //황팀추가
            return;
        }
    ...
    */
    ...
    if(isAndroid) {
		...
		$(document).on("touchend", ".it_supply", function() { //황팀 - 수정
            supply_add = true;
        });
    } else {
		...
		$(document).on(it_supply_events, ".it_supply", function(e) { //황팀 - 수정
            supply_add = true;
        });
    }
    ...
    $(document).on("click", "#sit_sel_option li button", function() {
        var $this = $(this),
			...
			$sit_opt_prc = $(this).closest("li").find(".sit_opt_prc"), //황팀 - 추가
			//$sit_opt_prc = $('.opt_count').find(".sit_opt_prc"), //황팀 - 추가
            stock = parseInt($(this).closest("li").find("input.io_stock").val()),
			io_price = parseInt($(this).closest("li").find("input.io_price").val()), //황팀 - 추가
			io_type = $(this).closest("li").find("input[name^=io_type]").val(),				
			it_price = parseInt($("input#it_price").val()); //황팀 - 추가
			
		//황팀 - 추가
		var option_price = 0;
		if(io_type == "0") { // 선택옵션
			option_price = it_price + io_price;			
		} else { // 추가옵션
			option_price = io_price;
		}
        ...
        switch(mode) {
            case "증가":
                ...
                $el_qty.val(this_qty);
				opt_prcs = '+' + number_format(String(option_price*this_qty))  +'원';
				$sit_opt_prc.html(opt_prcs); //황팀 - 추가
                $this.trigger("sit_sel_option_success", [$this, mode, this_qty, opt_prcs]);
                price_calculate();
                break;
...
{
	var it_price = parseInt($("input#it_price").val()); //황팀 - 추가
	...
	*/
	...
	if(type==0) {
		option_price = it_price + parseInt(price);
	}
	var opt_prc;
    if(parseInt(option_price) >= 0)
        opt_prc = "+"+number_format(String(option_price))+"원";
    else
        opt_prc = ""+number_format(String(option_price))+"원";
	...
	opt += "</div>";
	opt += "<div class='row'>";
	opt += "<div class='quantity-controler'>";
...
function price_calculate()
{
    ...
    var price, type, qty, total = 0, total_qty = 0;
	...
	//황팀 - 수정 (오더 총 수량 부분은 기존 소스에 없는 데이터라 임의로 넣었습니다.)
	//$("#sit_tot_price").empty().html('<div class="inner"><div class="order-total-quantity">총 수량<span class="result">'+number_format(String(total_qty))+'</span>개</div><div class="order-total-price">총 금액<span class="result">'+number_format(String(total))+'</span>원</div></div>');
