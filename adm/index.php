...
<section>
    ...
    <div class="tbl_head01 tbl_wrap">
		...
		$sql = "SELECT *
FROM (
select a.*, b.bo_subject, c.gr_subject, c.gr_id from g5_board_new a, g5_board b, g5_group c 
where a.bo_table = b.bo_table and b.gr_id = c.gr_id 

UNION all

select iq_id, 'shop_item_qa', iq_id, iq_subject, iq_time , q.mb_name , '상품문의', '상품문의', 'qa'
from g5_shop_item_qa a left join g5_shop_item b on (a.it_id = b.it_id) left join g5_member q on (a.mb_id = q.mb_id)  
WHERE iq_answer != ''
)u
ORDER BY bn_datetime DESC LIMIT 5";
//        $sql = " select a.*, b.bo_subject, c.gr_subject, c.gr_id {$sql_common} {$sql_order} limit {$new_write_rows} ";
        $result = sql_query($sql);
		https://kaviar.co.kr/shop/item.php?it_id=1712017374
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
			if($row['bo_table'] == "shop_item_qa"){

				$comment = $row['wr_parent'];
				$datetime= substr($row['bn_datetime'],0,10);
				$name = $row['mb_id'];

				$bo_url = "https://kaviar.co.kr/adm/shop_admin/itemuselist.php";
				$comment_url = "https://kaviar.co.kr/adm/shop_admin/itemqalist.php?iq_id=".$row['bn_id'];

			}else{
				$tmp_write_table = $g5['write_prefix'] . $row['bo_table'];
				$bo_url = get_pretty_url($row['bo_table']);
				...
				$comment_url = get_pretty_url($row['bo_table'], $row2['wr_id']).$comment_link;
			}
        ?>

        <tr>
            <td class="td_category">
				<a href="<?php echo G5_BBS_URL ?>/new.php?gr_id=<?php echo $row['gr_id'] ?>">
					<?php echo cut_str($row['gr_subject'],10) ?>
				</a>
			</td>
