...
<div id="sch_member_frm" class="new_win scp_new_win">
	...
	<div id="scp_list_find">
        <label for="mb_name">선택회원</label>
        <input type="text" name="select_id" id="select_id" class="frm_input" style="width:70%">
		<button type="button" class="btn btn_03" onclick="sel_member_id_out();">등록</button>
    </div>
	-->
	...
	<form name="fmember" method="get">
	<div id="scp_list_find">
        <label for="mb_name">회원등록 리스트</label>
        <input type="text" name="select_id" id="select_id" class="frm_input required" readonly size="40">
        <button type="button" class="btn btn_03" onclick="sel_member_id_out();">등록</button>
    </div>
...
function sel_member_id_out()
{
	var select_id = $("#select_id").val();
	
    var f = window.opener.document.fpointlist2;
    f.mb_id.value = select_id;
