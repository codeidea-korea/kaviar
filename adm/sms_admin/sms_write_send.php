...
if ($_POST['sms_use'] == 'aligo') {


	//aligo_sms_call($sms_content, $recv_number, $send_number, "", "", "");

	$wr_reply   = isset($_REQUEST['wr_reply']) ? preg_replace('#[^0-9\-]#', '', trim($_REQUEST['wr_reply'])) : ''; //보내는사람
	$wr_message = isset($_REQUEST['wr_message']) ? clean_xss_tags(trim($_REQUEST['wr_message']), 1, 1, 0, 0) : ''; //메세지
	$send_list = isset($_REQUEST['send_list']) ? clean_xss_tags(trim($_REQUEST['send_list']), 1, 1) : '';  //받는사람

	//echo "1 : ".$wr_reply."<br>2 : ".$wr_message."<br>3: ".$send_list;
		...
		for ($i=1, $max = count($item); $i<$max; $i++)
		{
			...
			switch ($item[0])
			{
				...
				case 'h': 
					...
					$qry = sql_fetch("select mb_id,mb_grade from {$g5['member_table']} where mb_hp = '".$item[$i][1]."' ");

					$hp = get_hp($item[$i][1], 0);
					$name = $item[$i][0];
					
					$m_result = aligo_sms_call($wr_message, $hp, $wr_reply, "", "", "");
					
					//echo "<br><br>".$m_result."<br><br>";
					$msg_result = explode('/', $m_result);
					
					

					sql_query("insert into {$g5['sms5_history_table']} set mb_id='".$qry['mb_id']."', mb_grade='".$qry['mb_grade']."', hs_hp='".$hp."', hs_datetime='".G5_TIME_YMDHIS."', hs_flag='".$msg_result[1]."', hs_code='".$msg_result[0]."', hs_memo='".addslashes($wr_message)."'" );
				...
				case 'l':
					...
					$qry = sql_query("select bk_hp from {$g5['sms5_book_table']} where mb_grade = $mb_level ");
					$qry1 = sql_query("select * from {$g5['sms5_book_table']} where mb_grade = $mb_level ");
					$send = "";
					for ($y=0; $roy=sql_fetch_array($qry); $y++) {
						if($y==0){
							$send = $roy['bk_hp'];
						}else {
							$send = $send.",".$roy['bk_hp'];
					...
					//echo "<br><br>send : ".$send."<br><br>";

					$sendlist = str_replace("-", "", $send);
					$m_result = aligo_sms_call($wr_message, $sendlist, $wr_reply, "", "", "");
					$msg_result = explode('/', $m_result);
					
					if($msg_result[1] == '1'){
						echo "<br> bb  : ".$msg_result[1]."<br>";

						for ($z=0; $roz=sql_fetch_array($qry1); $z++) {

							$hps = get_hp($roz['bk_hp'], 0);
						
							sql_query("insert into {$g5['sms5_history_table']} set mb_id='".$roz['mb_id']."', mb_grade='".$mb_level."', hs_hp='".$hps."', hs_datetime='".G5_TIME_YMDHIS."', hs_flag='".$msg_result[1]."', hs_code='".$msg_result[0]."', hs_memo='".addslashes($wr_message)."'" );
	...
	//order_email_call('주문완료', $_POST['od_email'], $member['mb_id'], $od_id, $od_name, G5_TIME_YMDHIS,  $default['de_admin_company_name'], $od_bank_account);

}else if ($_POST['sms_use'] == 'icode') {
