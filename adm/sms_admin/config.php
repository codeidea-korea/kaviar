...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr class="icode_json_version">
        <th scope="row"><label for="cf_aligo_token_key">아이코드 토큰키<br>(JSON버전)</label></th>
        <td>
            <input type="text" name="cf_aligo_token_key" value="<?php echo $config['cf_aligo_token_key']; ?>" id="cf_aligo_token_key" class="frm_input" size="40">
            <?php echo help("아이코드 사이트 -> 토큰키관리 메뉴에서 생성한 토큰키를 입력합니다."); ?>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="cf_aligo_sender">회신번호<strong class="sound_only"> 필수</strong></label></th>
        <td>
            <?php echo help("회신받을 휴대폰 번호를 입력하세요. 회신번호는 발신번호로 사전등록된 번호와 동일해야 합니다.<br>예) 010-123-4567"); ?>
            <input type="text" name="cf_aligo_sender" value="<?php echo isset($config['cf_aligo_sender']) ? get_sanitize_input($config['cf_aligo_sender']) : ''; ?>" id="cf_aligo_sender" required class="frm_input required" size="13">
        </td>
    </tr>
    </tbody>
    </table>
