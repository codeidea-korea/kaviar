...
//$qry = sql_query("select mb_grade, count(*) as cnt from {$g5['member_table']} where mb_sms=1 and not (mb_hp='') group by mb_grade");
$qry = sql_query("select mb_grade, count(*) as cnt from {$g5['sms5_book_table']} group by mb_grade ");

while ($row = sql_fetch_array($qry))
{
    $lev[$row['mb_grade']] = $row['cnt'];
}
$str_json = array();
$line = 0;
$tmp_str = '';
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tbody>';
    for ($i=1; $i<10; $i++) {
		$grade = sql_fetch(" select g_name from `g5_member_grade` where idx = $i ");
        $bg = 'bg'.($line++%2);
        $tmp_str .= '
        <tr class="'.$bg.'">
            <td>'.$grade['g_name'].'</td>
