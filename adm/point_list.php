...
<section id="point_mng">
    ...
    <form name="fpointlist2" method="post" id="fpointlist2" action="./point_update.php" autocomplete="off" onsubmit="return fpoint_submit(this);">
    <input type="hidden" name="sfl" value="<?php echo $sfl ?>">
    <input type="hidden" name="stx" value="<?php echo $stx ?>">
    <input type="hidden" name="sst" value="<?php echo $sst ?>">
    <input type="hidden" name="sod" value="<?php echo $sod ?>">
...
$(function() {
    ...
    $("#sch_member").click(function() {
		...
		for(let i = 2; i <= 8; i++) {
			if(document.getElementById(`all_mb${i}`).checked) {
				alert("등급 체크를 해제 후 이용해 주십시오.");
				return false;
			}
		}

        var opt = "left=50,top=50,width=520,height=600,scrollbars=1";
        var url = "./pointmember.php";
...
function fpoint_submit(f)
{
	...
	const result = confirm("포인트를 지급하시겠습니까?");
