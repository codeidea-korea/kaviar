...
<form name="fboardgroup" id="fboardgroup" action="<?=G5_ADMIN_URL?>/my/boardgroup_form_update.php" onsubmit="return fboardgroup_check(this);" method="post" autocomplete="off">
...
<section class="mybox blue">
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
			<th scope="row"><label>그룹 라벨</label></th>
			<td>
				<input type="text" name="gr_1" value="<?=get_text($group['gr_1'])?>" class="colorpicker" data-format="rgb" data-opacity="1" data-swatches="<?=$swathColor?>" placeholder="#">
			</td>
		</tr>
		...
		<tr>
			<th scope="row"><label>서브메뉴 사용</label></th>
			<td>
				<input type="checkbox" name="gr_2" value="1" id="gr_2" <?php echo $gr['gr_2']?'checked':''; ?> data-label="사용">
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<input type="checkbox" name="gr_use_access" value="1" id="gr_use_access" <?php echo $gr['gr_use_access']?'checked':''; ?> data-label="사용">
			</td>
		</tr>
		<tr>
...
<?php if($group['gr_id']) { ?>
<section class="mybox blue mt20 " style="background:rgba(59,118,295,0.02);">	
	<div class="formContainer label200">
		<div class="form-list <?=$group['gr_main_table']?"use":"noUse";?>">
			<div class="form-label" style="width:220px"><label style="font-size:14px;color:#3b76ff;"><?=$group['gr_subject']?> 전용 메인페이지</label></div>
			<div class="formCon">
				<?php echo get_board_select_my('gr_main_table', $group['gr_main_table'], 'class="selectpicker span300"', 'pageMake', 'subject', $group['gr_id']); ?>
				<?php if($group['gr_main_table']) echo '<a href="'.G5_BBS_URL.'/board.php?bo_table='.$group['gr_main_table'].'" target="_blank" class="btn_frmline" style="line-height:25px;display:inline-block;vertical-align:top;margin-left:5px;">게시판 바로가기</a>';?>
			</div>
		</div>
	</div>
</section>
