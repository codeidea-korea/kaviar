...
// 휴대폰번호 체크
if($_POST['mb_hp_orgin'] != $_POST['mb_hp']){
...
$mb_sexs        = isset($_POST['mb_sexs']) ? trim($_POST['mb_sexs']) : '';
$mb_buisness    = isset($_POST['mb_buisness']) ? trim($_POST['mb_buisness']) : '';

$mb_births	  = $mb_year."-".$mb_month."-".$mb_day;
$mb_buisness  = ($mb_buisness != '') ? 1 : "0";
...
$mb_grade = isset($_POST['mb_grade']) ? $_POST['mb_grade'] : $_POST['mb_grades'];
...
if( $w == '' || $w == 'u' ){
	...
	$mb_dir = substr($mb_id,0,2);
    $mb_img = get_mb_icon_name($mb_id).'.gif';
    ...
    if (isset($del_mb_img) && $del_mb_img) {
		...
		while ($entry = $d->read()) {
			...
			if(strpos($entry, 'thumb-'.get_mb_icon_name($mb_id).'_') !== false) {
				@unlink($mb_img_dir."/".$entry);
			}
		}
	}
    ...
    if (isset($_FILES['mb_img']) && is_uploaded_file($_FILES['mb_img']['tmp_name'])) {
        ...
        if (preg_match($image_regex, $_FILES['mb_img']['name'])) {
            ...
            if (file_exists($dest_path)) {
                $size = @getimagesize($dest_path);
				$member_img_size = 150;
                if ($size[0] > $member_img_size || $size[1] > $member_img_size) {
                    $thumb = null;
                    if($size[2] === 2 || $size[2] === 3 || $size[2] === 18) { // WebP 추가
                        //jpg 또는 png 파일 적용
                        $thumb = thumbnail($mb_img, $mb_img_dir, $mb_img_dir, $member_img_size, $member_img_size, true, true);
                        if($thumb) {
                            @unlink($dest_path);
...
goto_url(G5_ADMIN_URL.'/member_form.php?'.$qstr.'&amp;w=u&amp;mb_id='.$mb_id, false);