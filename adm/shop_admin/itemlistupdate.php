...
if ($post_act_button == "선택수정") {
    ...
    for ($i=0; $i< $count_post_chk; $i++) {
		...
		$p_it_shpping_name = (isset($_POST['it_shpping_name']) && is_array($_POST['it_shpping_name'])) ? strip_tags($_POST['it_shpping_name'][$k]) : '';
		
		$p_s_start_date = (isset($_POST['it_sale_start_date']) && is_array($_POST['it_sale_start_date'])) ? strip_tags($_POST['it_sale_start_date'][$k]) : '';
		$p_s_end_date = (isset($_POST['it_sale_end_date']) && is_array($_POST['it_sale_end_date'])) ? strip_tags($_POST['it_sale_end_date'][$k]) : '';
        ...
        $sql = "update {$g5['g5_shop_item_table']}
                   set ca_id          = '".sql_real_escape_string($p_ca_id)."',
					   ...
					   it_sale_start_date       = '".sql_real_escape_string($p_s_start_date)."',
					   it_sale_end_date       = '".sql_real_escape_string($p_s_end_date)."',
                       it_update_time = '".G5_TIME_YMDHIS."',
					   it_shpping_name = '".sql_real_escape_string($p_it_shpping_name)."'
                 where it_id   = '".$p_it_id."' ";
