...
// 알리고 토큰키 추가
if( ! isset($config['cf_aligo_token_key']) ){
    $sql = "ALTER TABLE `{$g5['config_table']}` 
            ADD COLUMN `cf_aligo_token_key` VARCHAR(100) NOT NULL DEFAULT '' AFTER `cf_icode_token_key`; ";
    sql_query($sql, false);
}

// 알리고 발송전화번호 추가
if( ! isset($config['cf_aligo_sender']) ){
    $sql = "ALTER TABLE `{$g5['config_table']}` 
            ADD COLUMN `cf_aligo_sender` VARCHAR(100) NOT NULL DEFAULT '' AFTER `cf_aligo_token_key`; ";
    sql_query($sql, false);
}

// 네이버 발송전화번호 추가
if( ! isset($config['cf_naver_sender']) ){
    $sql = "ALTER TABLE `{$g5['config_table']}` 
            ADD COLUMN `cf_naver_sender` VARCHAR(100) NOT NULL DEFAULT '' AFTER `cf_aligo_sender`; ";
    sql_query($sql, false);
}

/* 2024-02-21 준섭 알리고,네이버 끝 */
