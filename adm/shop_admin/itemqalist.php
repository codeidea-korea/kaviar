...
$sql_date = " and iq_time between date('{$fr_date}') AND DATE_ADD(DATE('{$to_date}'), INTERVAL 1 DAY) ";


$sql_search = " where (1) ";

if($iq_id){
	$sql_search .= " and iq_id = '$iq_id' ";
}else{
	if ($stx != "") {
		if ($sfl != "") {
	...
	$order_by = "order by $sst $sod , iq_id desc";
...
<section class="mybox" style="padding:6px !important">
    <div class="tbl_frm01 tbl_wrap" style="margin:0px !important">
		...
		<tr>
			<th scope="row"><label for="cf_admin">문의담당자 연락처</label></th>
            <td><input type="text"  onchange="g_save(3,this.value)" name="cf_manager_hp_qna" value="<?=$config['cf_manager_hp_qna']?>" id="cf_manager_hp_qna" class="frm_input" size="20" placeholder="담당자 전화번호"></td>
			<th scope="row"><label for="cf_admin">문의담당자 이메일</label></th>
            <td><input type="text"  onchange="g_save(4,this.value)" name="cf_manager_email_qna" value="<?=$config['cf_manager_email_qna']?>" id="cf_manager_email_qna" class="frm_input" size="20" placeholder="담당자 이메일"></td>
        </tr>
...
<div class="tbl_head01 tbl_wrap" id="itemqalist">
    ...
    <tr>
		...
		<th scope="col"><?php echo subject_sort_link('iq_time'); ?>등록일자</a></th>
        <th scope="col">관리</th>
    </tr>
    </thead>
    <tbody>
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td class="td_name" style="text-align:center !important"><?php echo $row['iq_time']; ?></td>
        <td class="td_mng td_mng_s">
            <a href="./itemqaform.php?w=u&amp;iq_id=<?php echo $row['iq_id']; ?>&amp;<?php echo $qstr; ?>" class="btn btn_03" target="_blink" rel="noreferrer noopener"><span class="sound_only"><?php echo get_text($row['iq_subject']); ?> </span>수정</a>
        </td>
    </tr>
    <?php
    }
