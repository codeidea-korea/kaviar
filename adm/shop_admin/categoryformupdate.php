...
$sql_common = " ca_order                = '$ca_order',
                ...
                ca_9                    = '$ca_9',
                ca_10                   = '$ca_10',
				ca_time				= '".G5_TIME_YMDHIS."' ";

$sql_common .= ", ca_menu_use                = '$ca_menu_use' ";
...
else if ($w == "d")
{
	...
	//분류 삭제시 이미지도 삭제
	@unlink(G5_DATA_PATH."/shop_cate/$ca_id");
	$files = glob(G5_DATA_PATH."/shop_cate/thumb-$ca_id*");
	if (is_array($files)) {
...
if(is_uploaded_file($_FILES['shop_cate_img']['tmp_name'])) {
	$files = glob(G5_DATA_PATH."/shop_cate/thumb-$ca_id*");
	if (is_array($files)) {
		foreach($files as $thumbnail) {
			$cnt++;
	...
	if (!preg_match($image_regex, $_FILES['shop_cate_img']['name']))
		alert($_FILES['shop_cate_img']['name'] . '은(는) 이미지 파일이 아닙니다.');
	if (preg_match($image_regex, $_FILES['shop_cate_img']['name'])) {
		$dest_path = $img_path.'/'.$ca_id;
		move_uploaded_file($_FILES['shop_cate_img']['tmp_name'], $dest_path);
