...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$c_cnt = sql_fetch(" select count(*) as cnt from `g5_shop_coupon_log` where cz_id = '".$row['cz_id']."' ");
    ?>
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td class="td_left"><?php echo get_text($row['cz_code']); ?></td>
	    ...
	    <td class="td_num"><?php echo number_format($c_cnt['cnt']); ?></td>
        <td class="td_datetime"><?php echo substr($row['cz_start'], 2, 8); ?> ~ <?php echo substr($row['cz_end'], 2, 8); ?></td>
        <td class="td_mng td_mng_s">
            <a href="./couponzoneform.php?w=u&amp;cz_id=<?php echo $row['cz_id']; ?>&amp;<?php echo $qstr; ?>" class="btn btn_03" target="_blink" rel="noreferrer noopener"><span class="sound_only"><?php echo get_text($row['cz_subject']); ?> </span>수정</a>
        </td>
    </tr>
