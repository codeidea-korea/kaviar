...
for ($i=0; $i<$count_post_chk; $i++)
{
    ...
    $current_status = $od['od_status']; //현재
    $change_status  = isset($_POST['od_status']) ? clean_xss_tags($_POST['od_status'], 1, 1) : ''; // 바뀔놈
    ...
    switch ($current_status)
    {
        case '주문' :
			...
			$receive_numbers = $od['od_hp'];
			
			$receive_number = preg_replace("/[^0-9]/", "", $receive_numbers);
			$od_total_price = $od['od_cart_price'] + $od['od_send_cost'] + $od['od_send_cost2'];

			$sms_content7 = str_replace("{이름}", $od['od_name'], $sms_content7);
			$sms_content7 = str_replace("{회원아이디}", $od['mb_id'], $sms_content7);
			$sms_content7 = str_replace("{회사명}", $default['de_admin_company_name'], $sms_content7);
			$sms_content7 = str_replace("{주문번호}", $od['od_id'], $sms_content7);
			$sms_content7 = str_replace("{주문금액}", number_format($od_total_price), $sms_content7);
			...
			if($config['cf_sms_use'] == "aligo"){
				...
				order_email_call('입금완료', $od['od_email'], $od['mb_id'], $od['od_id'], $od['od_name'], $od['od_time'],  $default['de_admin_company_name'], $default['de_bank_account'], $sms_content7);

				//알림톡발송
				//$tpl_code, $recevier, $name="", $od_id="", $od_price="", $product="", $md_id="", $account="", $company="", $invoice=""
				kakao_alim("TS_0380", $receive_number, $od['od_name'], $od['od_id'], $od_total_price, "", "", "", "", "");
			...
			$order_grade_mdid = $od['mb_id'];			//구매자 아이디
			//주문완료된 금액 확인하기
		...
		*/
        ...
        case '준비' :
			...
			$sms_content8 = str_replace("{이름}", $od['od_name'], $sms_content8);
			$sms_content8 = str_replace("{회원아이디}", $od['mb_id'], $sms_content8);
			$sms_content8 = str_replace("{회사명}", $default['de_admin_company_name'], $sms_content8);
			$sms_content8 = str_replace("{주문번호}", $od['od_id'], $sms_content8);
			$sms_content8 = str_replace("{주문금액}", number_format($od_total_price)."원", $sms_content8);
			$sms_content8 = str_replace("{주문일자}", $od['od_time'], $sms_content8);
			$sms_content8 = str_replace("{배송사}", $od['od_delivery_company'], $sms_content8);
			$sms_content8 = str_replace("{송장번호}", $od['od_invoice'], $sms_content8);
			...
			if($config['cf_sms_use'] == "aligo"){
				...
				if($od['od_delivery_company'] && $od['od_invoice']){
					...
					order_email_call('배송중', $od['od_email'], $od['mb_id'], $od['od_id'], $od['od_name'], $od['od_time'],  $default['de_admin_company_name'], $default['de_bank_account'], $sms_content8);

					//알림톡발송
					//$tpl_code, $recevier, $name="", $od_id="", $od_price="", $product="", $md_id="", $account="", $company="", $invoice=""
					kakao_alim("TS_0381", $receive_number, $od['od_name'], "", "", "", "", "", $od['od_delivery_company'], $od['od_invoice']);
				}
		...
		*/
        ...
        case '배송' :
            ...
            */
			...
			$sms_content11 = str_replace("{이름}", $od['od_name'], $sms_content11);
			$sms_content11 = str_replace("{회원아이디}", $od['mb_id'], $sms_content11);
			$sms_content11 = str_replace("{회사명}", $default['de_admin_company_name'], $sms_content11);
			$sms_content11 = str_replace("{주문번호}", $od['od_id'], $sms_content11);
			$sms_content11 = str_replace("{주문금액}", number_format($od_total_price."원"), $sms_content11);
			$sms_content11 = str_replace("{주문일자}", $od['od_time'], $sms_content11);
			...
			if($config['cf_sms_use'] == "aligo"){
				...
				order_email_call('배송완료', $od['od_email'], $od['mb_id'], $od['od_id'], $od['od_name'], $od['od_time'],  $default['de_admin_company_name'], $default['de_bank_account'], $sms_content11);
				
				//알림톡발송
				//$tpl_code, $recevier, $name="", $od_id="", $od_price="", $product="", $md_id="", $account="", $company="", $invoice=""
				kakao_alim("TS_0385", $od['od_hp'], $od['od_name'], $od['od_id'], "", "", "", "", "", "");
	...
	//완료일 업데이트하기
	$od_result_date = ($change_status == "완료")?G5_TIME_YMDHIS:"0000-00-00 00:00:00";
	...
	if($change_status == "완료"){
		...
		$k     = isset($_POST['chk'][$i]) ? $_POST['chk'][$i] : 0;
		$list_mb_id = isset($_POST['list_mb_id'][$k]) ? clean_xss_tags($_POST['list_mb_id'][$k], 1, 1) : '';
		$list_tot_price = isset($_POST['list_tot_price'][$k]) ? clean_xss_tags($_POST['list_tot_price'][$k], 1, 1) : '';

		//입금완료시 등급조절하기  //준섭
		$order_grade_price = $list_tot_price; 	//주문한금액 $_POST['od_total_price']
		$order_grade_mdid = $list_mb_id;			//구매자 아이디
		//주문완료된 금액 확인하기
		$result_price = sql_fetch(" select sum(od_cart_price + od_send_cost) as od_price from `g5_shop_order` where od_status = '완료' and mb_id = '".$order_grade_mdid."' ");
		//현재등급 가져오기
		$mb_grades = sql_fetch(" select mb_grade,mb_order_price from `g5_member` where mb_id = '".$order_grade_mdid."' ");
