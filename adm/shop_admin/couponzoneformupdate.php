...
$sql_code = " select count(*) as cnt from {$g5['g5_shop_coupon_zone_table']} where cz_code = '".$_POST['cz_code']."' ";
$row_code = sql_fetch($sql_code);
if($row_code['cnt']){
	alert("동일한 쿠폰 번호를 사용하실수 없습니다");
	return false;
}
...
if($w == '') {
    //if(!$_FILES['cp_img']['name'])
        //alert('쿠폰이미지를 업로드해 주십시오.'); //인태 - 쿠폰이미지 없이 등록 가능하도록
/*
	$j = 0;
    do {
        $cp_id = get_coupon_id();

        $sql3 = " select count(*) as cnt from {$g5['g5_shop_coupon_table']} where cz_code = '$cp_id' ";
        $row3 = sql_fetch($sql3);
