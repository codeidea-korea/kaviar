...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
		...
		<?php for ($t=0; $t < count($itemtype); $t++) {
			$num = $t + 1;
			if($itemtype[$t]) echo '<th scope="col">'.subject_sort_link("it_type".$num, $qstr, 1).$itemtype[$t].'</a></th>';
		} ?>
		<!--
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<?php for ($t=0; $t < count($itemtype); $t++) {
			...
			if($itemtype[$t]) {
				...
				echo '<label for="type'.$num.'_'.$i.'" class="sound_only">'.$itemtype[$t].'</label>';
				echo '<input type="checkbox" name="it_type'.$num.'['.$i.']" value="1" id="type1_'.$i.'" '.($row['it_type'.$num] ? 'checked' : '').'>';
				echo '</td>';
			}
		} ?>
		
        <td class="td_mng td_mng_s">
            <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>" class="btn btn_03" target="_blink" rel="noreferrer noopener"><span class="sound_only"><?php echo cut_str(stripslashes($row['it_name']), 60, "&#133"); ?> </span>수정</a>
         </td>
    </tr>
    <?php
    }
