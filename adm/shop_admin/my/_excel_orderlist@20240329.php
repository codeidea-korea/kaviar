...
<tr>
	...
	<td>
		<?php 
			$optionsql1 = " select it_id from {$g5['g5_shop_cart_table']} where od_id = '{$row['od_id']}' group by it_id";

            $res1 = sql_query($optionsql1);
			for($g=0; $res1=sql_fetch_array($res1); $g++) {
                echo '<br>'.get_text($res1['it_id']);
			 }

		?>
	</td><!-- 상품코드 -->
	<td><?php echo $row['od_cart_count']; ?>건</td> <!-- 주문 수량 -->
	<td></td> <!-- 회원할인 -->
	<td><?php echo number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']); ?></td> <!-- 총결제금액 -->
	<td><?php echo number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']); ?></td> <!-- 실결제금액 -->
	<td><?php echo $row['od_point']; ?></td> <!-- 사용적립금 -->
	<td><?php echo number_format($row['couponprice']); ?></td> <!-- 사용쿠폰명 -->
	<td></td> <!-- 사용예치금 -->
	<td><?php echo number_format($row['od_send_cost'] + $row['od_send_cost2']); ?></td> <!-- 배송비-->
	...
	<td><?=$row['od_invoice'] ? $row['od_invoice'] : '-'?></td> <!-- 운송장번호 -->
	<td><?=$row['od_delivery_company'] ? $row['od_delivery_company'] : '-'?></td> <!-- 배송회사 -->
	<td><?=is_null_time($row['od_invoice_time']) ? '-' : substr($row['od_invoice_time'],2,14)?></td> <!-- 배송일시 -->
	<td><?php echo number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']); ?></td> <!--주문합계 배송비포함-->
	<td><?php echo number_format($row['od_receipt_price']); ?></td> <!-- 입금합계 -->
	<td><?php echo number_format($row['od_cancel_price']); ?></td> <!-- 주문취소 -->
	<td><?php echo number_format($row['couponprice']); ?></td> <!-- 쿠폰 -->
	<td><?php echo number_format($row['od_misu']); ?></td> <!-- 미수금 -->
</tr>
