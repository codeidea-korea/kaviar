...
if (phpversion() >= '5.2.0') {
    ...
    for($i=1; $row=sql_fetch_array($result); $i++) {
		...
		$itemsql = " select io_price, ct_id, ct_status, it_id, it_name,ct_qty,ct_price,ct_point,ct_invoice_number from {$g5['g5_shop_cart_table']} where od_id = '{$row['od_id']}' order by ct_id ";
		...
		for($k=0; $ite=sql_fetch_array($itemresult); $k++) {	
			...
			$addrs = trim($row['od_b_addr1']." ".$row['od_b_addr2']." ".$row['od_b_addr3']);
    ...
    for($i=1; $row=sql_fetch_array($result); $i++) {
        ...
        $worksheet->write($i, 7, print_address($addrs, $row['od_b_addr_jibeon']));
