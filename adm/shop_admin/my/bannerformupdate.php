...
$bn_bimg2      = isset($_FILES['bn_bimg2']['tmp_name']) ? $_FILES['bn_bimg2']['tmp_name'] : '';
$bn_bimg2_name = isset($_FILES['bn_bimg2']['name']) ? $_FILES['bn_bimg2']['name'] : '';
...
$bn_cate = isset($_POST['bn_cate']) ? clean_xss_tags($_POST['bn_cate'], 1, 1) : '';
...
if ($bn_bimg2_del)  @unlink(G5_DATA_PATH."/banner/{$bn_id}_2");

//파일이 이미지인지 체크합니다.
if( $bn_bimg || $bn_bimg_name ){

    if( !preg_match('/\.(gif|jpe?g|bmp|png|webp)$/i', $bn_bimg_name) ){
...
if ($w=="") {
	...
	@unlink(G5_DATA_PATH."/banner/{$bn_id}_2");

	$files = glob(G5_DATA_PATH."/banner/thumb-$bn_id*");
    if (is_array($files)) {
...
if ($w=="c") {
    ...
    $sql = " insert into {$g5['g5_shop_banner_table']}
                set bn_alt        = '{$bn['bn_alt']}',
                    bn_url        = '{$bn['bn_url']}',
                    bn_device     = '{$bn['bn_device']}',
                    bn_position   = '{$bn['bn_position']}',
					bn_cate   = '{$bn['bn_cate']}',
                    bn_border     = '{$bn['bn_border']}',
                    bn_new_win    = '{$bn['bn_new_win']}',
                    bn_begin_time = '{$bn['bn_begin_time']}',
                    bn_end_time   = '{$bn['bn_end_time']}',
                    bn_time       = '".G5_TIME_YMDHIS."',
                    bn_hit        = '0',
                    bn_order      = '{$bn['bn_order']}',
					bn_log      = '$bn_log' ";
    sql_query($sql);

    $new_bn_id = sql_insert_id();

	copy(G5_DATA_PATH."/banner/$bn_id", G5_DATA_PATH."/banner/$new_bn_id");
	copy(G5_DATA_PATH."/banner/{$bn_id}_2", G5_DATA_PATH."/banner/{$new_bn_id}_2");
...
if ($w == "" || $w == "u") {
	...
	if ($_FILES['bn_bimg2']['name']) upload_file($_FILES['bn_bimg2']['tmp_name'], $bn_id.'_2', G5_DATA_PATH."/banner");

    goto_url(G5_ADMIN_URL."/shop_admin/bannerform.php?w=u&amp;bn_id=$bn_id");
} else {
    goto_url(G5_ADMIN_URL."/shop_admin/bannerlist.php");
}