...
<section class="mybox" style="padding:6px !important">
    <div class="tbl_frm01 tbl_wrap" style="margin:0px !important">
        <table>
		...
		<tr>
            <th>주문내역 담당자 연락처</th>
            <td><input type="text" onchange="g_save(5,this.value)" name="cf_manager_order_hp" value="<?=$config['cf_manager_order_hp']?>" id="cf_manager_order_hp" class="frm_input" size="20" placeholder="주문내역 담당자 전화번호"></td>
			<th scope="row"><label for="cf_admin">주문내역 담당자 이메일</label></th>
            <td><input type="text"  onchange="g_save(6,this.value)" name="cf_manager_order_email" value="<?=$config['cf_manager_order_email']?>" id="cf_manager_order_email" class="frm_input" size="20" placeholder="주문내역 담당자 이메일"></td>
        </tr>
...
<div>
	<span style="font-weight:bold">자동배송완료 일자 :</span>
	<input type="input" name="auto_done" value="<?=$config['cf_auto_done']?>" onchange="f_auto_done(this.value)" style="width:25px;height:25px;;text-align:center;" maxlength="1">일
	
	<span style="padding-left:20px;font-weight:bold">현재상태 :</span>
	<select id="status_origin" style="font-size:12px !important;">
	...
	<span style="padding-left:10px;font-weight:bold">변경상태 :</span>
	...
	<span style="padding-left:10px;">
		<input type="button" onclick="all_change_status()"value="일괄변경 하기" class="btn btn_02" style="background:#ff4081;color:white">
	</span>
</div>
...
{
	var status_origin = $("#status_origin").val();
	var status_change = $("#status_change").val();
	
	if(status_change == status_origin){
		alert("상태값이 동일 할 수 없습니다.");
		return false;
	}

	var con_test = confirm("[" + status_origin + "] 상태를 [" + status_change +"] 상태로 일괄 변경 하시겠습니까?\n한번 실행 후 복구가 되지않습니다.");

	if(con_test == true){

	   $.ajax({
		  url:'ajax_status_all_chage.php',
		  type:'POST',
		  data : {status_origin : status_origin, status_change : status_change},
		  cache: false,
		  async: false,
		  dataType : 'json',
		  success: function(res) {
...
<div>
	...
	<input type="radio" name="od_status" value="배송보류" id="od_status_done" <?php echo get_checked($od_status, '배송보류'); ?>>
    <label for="od_status_done">배송보류</label>
...
<div class="flex flex-right mb15">
	<a href="<?=G5_ADMIN_URL?>/shop_admin/my/_excel_orderlist.php<?=$btn_qstr?>" class="btn_excel_download" target="_blank">엑셀 다운로드</a>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
		...
		<td headers="th_odrer" class="td_name">
			<?php 
				$mbs = sql_fetch(" select mb_grade from `g5_member` where mb_id = '".$row['mb_id']."' ");
				$grade = sql_fetch(" select g_name from `g5_member_grade` where idx = '".$mbs['mb_grade']."' ");
				echo $grade['g_name'];
			?>
		</td>
        ...
        <td rowspan="3" class="td_num_right"><?php echo number_format($row['od_receipt_price'] - $row['od_cancel_price']); ?></td>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="delino" class="delino">
			<?php
				$invice_cnt = sql_fetch(" SELECT COUNT(*) AS cnt FROM ( select count(*) as cnt from `g5_shop_cart` where od_id = '".$row['od_id']."' and ct_invoice_number != '' GROUP BY ct_invoice_number )a ");
				echo $invice_cnt['cnt'] ? $invice_cnt['cnt']."건" : $row['od_invoice'] ?  $row['od_invoice'] : "-";
			?>
            <?php //if ($od_status == '준비') { ?>
