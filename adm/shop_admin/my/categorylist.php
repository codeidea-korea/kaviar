...
<div id="sct" class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" colspan="2" id="sct_cate"><?php echo subject_sort_link("ca_name"); ?>분류명</a></th>
        <th scope="col" id="sct_amount">상품수</th>
        <th scope="col" id="sct_hpcert">본인인증</th>
        <th scope="col" id="sct_imgw">이미지 폭</th>
		<th scope="col" id="sct_imgw">모바일 이미지 폭</th>
    ...
    <tr>
        <th scope="col" colspan="2" id="sct_admin"><?php echo subject_sort_link("ca_mb_id"); ?>관리회원아이디</a></th>
        <th scope="col" id="sct_sell"><?php echo subject_sort_link("ca_use"); ?>판매가능</a></th>
        <th scope="col" id="sct_adultcert">카테고리 메뉴사용</th>
        <th scope="col" id="sct_imgh">이미지 높이</th>
		<th scope="col" id="sct_imgh">모바일 이미지 높이</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $s_level = '<div class="relative"><label for="ca_name_'.$i.'" '.$class.'><span class="sound_only">'.$p_ca_name.''.($level+1).'단 분류</span></label></div>';
        ...
        $s_upd = '<a href="./categoryform.php?w=u&amp;ca_id='.$row['ca_id'].'&amp;'.$qstr.'" class="btn btn_02" target="_blink" rel="noreferrer noopener"><span class="sound_only">'.get_text($row['ca_name']).' </span>수정</a> ';
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td class="td_code" rowspan="2">
			<?php
			$img_path[$i] = G5_DATA_PATH.'/shop_cate/'.$row['ca_id'];
			if(file_exists($img_path[$i])) {
				$ca_thumb[$i] = thumbnail($row['ca_id'], G5_DATA_PATH.'/shop_cate/', G5_DATA_PATH.'/shop_cate/', 100, '', 1, 1, 'center');
				$ca_img[$i] = '<img src="'.G5_DATA_URL.'/shop_cate/'.$ca_thumb[$i].'?'.preg_replace('/[^0-9]/i', '', $row['ca_time']).'">';
			}
			echo $ca_img[$i];
			?>
        </td>
        <td headers="sct_cate" class="sct_name<?php echo $level; ?>">
			<?php echo $s_level; ?>
			<input type="text" name="ca_name[<?php echo $i; ?>]" value="<?php echo get_text($row['ca_name']); ?>" id="ca_name_<?php echo $i; ?>" required class="tbl_input full_input required">
		</td>
		...
		<td headers="sct_imgw">
            <label for="ca_out_width<?php echo $i; ?>" class="sound_only">모바일 출력이미지 폭</label>
            <input type="text" name="ca_mobile_img_width[<?php echo $i; ?>]" value="<?php echo get_text($row['ca_mobile_img_width']); ?>" required class="required tbl_input" size="3" > <span class="sound_only">픽셀</span>
        </td>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="sct_adultcert" class="td_possible">
            <input type="checkbox" name="ca_menu_use[<?php echo $i; ?>]" value="1" id="ca_menu_use_yes<?php echo $i; ?>" <?php if($row['ca_menu_use']) echo 'checked="checked"'; ?>>
            <label for="ca_menu_use_yes<?php echo $i; ?>">사용</label>
        </td>
        <td headers="sct_imgh">
