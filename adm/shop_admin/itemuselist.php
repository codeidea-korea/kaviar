...
$sql_date = " and a.is_time between date('{$fr_date}') AND DATE_ADD(DATE('{$to_date}'), INTERVAL 1 DAY) ";
...
<section class="mybox" style="padding:6px !important">
    <div class="tbl_frm01 tbl_wrap" style="margin:0px !important">
		...
		<tr>
            <th>후기담당자 연락처</th>
            <td><input type="text" onchange="g_save(1,this.value)" name="cf_manager_hp" value="<?=$config['cf_manager_hp']?>" id="cf_manager_hp" class="frm_input" size="20" placeholder="담당자 전화번호"></td>
			<th scope="row"><label for="cf_admin">후기담당자 이메일</label></th>
            <td><input type="text"  onchange="g_save(2,this.value)" name="cf_manager_email" value="<?=$config['cf_manager_email']?>" id="cf_manager_email" class="frm_input" size="20" placeholder="담당자 이메일"></td>
        </tr>
...
<section class="mybox" style="margin-top:15px;margin-bottom:20px;padding:6px !important">
    <div class="formContainer label140">
        <div class="form-list">
            <div class="form-label"><label>사진 후기 페이지</label></div>
            <div class="formCon">
				<select onchange="g_save(7,this.value)" style="width:100px">
					<option value=""<?=(!$default['itemreview_skin']?" selected":"")?>>갤러리형</option>
					<option value="_wz"<?=($default['itemreview_skin']=='_wz'?" selected":"")?>>웹진형</option>
				</select>
				<a href="<?=G5_SHOP_URL?>/itemreview.php" class="_btn/sm ml10" target="_blank">사진후기 바로가기</a>
            </div>
        </div>
	</div>
</section>
...
<div class="tbl_head01 tbl_wrap" id="itemuselist">
    ...
    <tr>
		...
		<th scope="col"><?php echo subject_sort_link("is_time"); ?>등록일자</a></th>
        <th scope="col"><?php echo subject_sort_link("is_score"); ?>평점</a></th>
		<th scope="col">이미지<br>유무</a></th>
    ...
    <tr class="<?php echo $bg; ?>">
        <td class="td_chk">
			...
			<input type="hidden" name="is_file[<?php echo $i; ?>]" value="<?php echo $row['is_file']; ?>">
        </td>
        ...
        <td class="td_mng td_mng_s">
            <a href="./itemuseform.php?w=u&amp;is_id=<?php echo $row['is_id']; ?>&amp;<?php echo $qstr; ?>" class="btn btn_03" target="_blink" rel="noreferrer noopener"><span class="sound_only"><?php echo get_text($row['is_subject']); ?> </span>수정</a>
        </td>
    </tr>
