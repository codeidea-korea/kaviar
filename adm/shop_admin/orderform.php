...
<section id="anc_sodr_list">
    ...
    <input type="hidden" name="od_time" value="<?php echo substr($od['od_time'], 0, 16).'('.get_yoil($od['od_time']).')' ?>">
    <input type="hidden" name="od_total_price" value="<?php echo $od['od_cart_price'] + $od['od_send_cost'] + $od['od_send_cost2'] ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // 상품의 옵션정보
            $sql = " select ct_id, it_id, ct_price, ct_point, ct_qty, ct_option, ct_status, cp_price, ct_stock_use, ct_point_use, ct_send_cost, io_type, io_price,ct_invoice_number
                        from {$g5['g5_shop_cart_table']}
                        where od_id = '{$od['od_id']}'
                          and it_id = '{$row['it_id']}'
            ...
            for($k=0; $opt=sql_fetch_array($res); $k++) {
                ...
                $ct_point['stotal'] = $opt['ct_point'];
				//$ct_point['stotal'] = $opt['ct_point'] * $opt['ct_qty'];//준섭수정
            ?>
            <tr>
                <?php if($k == 0) { ?>
				...
				<td class="td_num_right" style="width:100px"><?php echo $opt['ct_invoice_number']; ?></td>
            </tr>
            <?php
                $chk_cnt++;
            }
...
<section id="anc_sodr_pay">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
			...
			<td class="td_numbig td_numsum"><?php echo number_format($amount['order']-$amount['cancel']); ?>원</td>
        </tr>
		<tr>
            <td colspan="9" style="text-align:right;padding-right:20px">
			<?php
				$coupon_sql = " select * from {$g5['g5_shop_coupon_log_table']} where od_id = '{$od['od_id']}'";
				$coupon_result = sql_query($coupon_sql);
				for($c=0; $crow=sql_fetch_array($coupon_result); $c++) {
					if($c==0){
						echo "사용쿠폰 이름 : ".$crow['cz_subject'];
					}else{
						echo ' / '.$crow['cz_subject'];
					}
    ...
    <div class="compare_wrap">
        ...
        <section id="anc_sodr_chk" class="compare_left">
            ...
            <div class="tbl_frm01">
				...
				<tr>
                    <th scope="row"><label for="od_send_mail">희망배송일</label></th>
                    <td>
						<input type="date" name="od_delivery_date" value="<?php echo $od['od_delivery_date']; ?>">
                        <?php echo help("희망배송일자 값이 저장될 경우,\n해당 주문의 상태는 '배송보류'로 변경됩니다."); ?>
                    </td>
                </tr>
                <?php
