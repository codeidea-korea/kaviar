...
@include_once($board_pcskin_path.'/good.head.skin.php');

//인태 - 비회원 추천일때 아이피로 중복채크 ─────────────────────────────────
if($board['bo_use_good_guest']) { 
	//비회원 추천
	if (!$member['mb_id']) {
		// $member['mb_id'] = $_SERVER['REMOTE_ADDR'];
		$member['mb_id'] = get_ip_address();
		$is_member = 1;
	}
}
...
if(isset($_POST['js']) && $_POST['js'] === "on") {
    ...
    //$ss_name = 'ss_view_'.$bo_table.'_'.$wr_id;
    //if (!get_session($ss_name))
        //alert('해당 게시물에서만 추천 또는 비추천 하실 수 있습니다.'); //인태 - 목록페이지에서 추천 가능하도록
