...
if (G5_IS_MOBILE) {
	...
	if(!$bo_top_img) echo html_purifier(stripslashes($board['bo_mobile_content_tail'])); //인태 - 상단이미지가 없을때만 출력 $bo_top_img

    // 모바일의 경우 설정을 따르지 않는다.
    include_once(G5_BBS_PATH.'/_tail.php');	
} else {
    //echo html_purifier(stripslashes($board['bo_content_tail']));
	if(!$bo_top_img) echo html_purifier(stripslashes($board['bo_content_tail'])); //인태 - 상단이미지가 없을때만 출력 $bo_top_img
