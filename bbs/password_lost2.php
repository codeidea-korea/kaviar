...
//인태
if(file_exists(G5_THIS_PATH.'/bbs/password_lost2.php')) {
	require_once(G5_THIS_PATH.'/bbs/password_lost2.php');
	return;
}
...
$sql = " select mb_id, mb_hp from {$g5['member_table']} where replace(mb_hp,'-','') = '".$_POST['mb_phone']."' limit 1";
$rrr = sql_fetch($sql);
...
*/
$sql = " select mb_hp, mb_no, mb_id, mb_name, mb_nick, mb_email, mb_datetime, mb_leave_date from {$g5['member_table']} where mb_id = '".$rrr['mb_id']."' ";
$mb = sql_fetch($sql);
...
$sms_contents = str_replace("{이름}", $mb['mb_name'], $sms_contents);
$sms_contents = str_replace("{회원아이디}", $mb['mb_id'], $sms_contents);
$sms_contents = str_replace("{회사명}", $default['de_admin_company_name'], $sms_contents);
$sms_contents = str_replace("{회원비밀번호}", $change_password, $sms_contents);


//이메일발송
pass_email_call($mb['mb_name'], $mb['mb_id'], $href, $change_password, $mb['mb_email'], $default['de_admin_company_name'],$sms_contents);


// 핸드폰번호에서 숫자만 취한다
$receive_number = preg_replace("/[^0-9]/", "", $mb['mb_hp']);  // 수신자번호 (회원님의 핸드폰번호)
$send_number = preg_replace("/[^0-9]/", "", $config['cf_aligo_sender']); // 발신자번호

//sms발송
aligo_sms_call($sms_contents, $receive_number, $send_number, "", "", "");
