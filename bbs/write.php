...
if ($board['bo_use_category']) {
	...
	if($w == '') { //인태 - 신규 등록시 분류가 선택된 채로 시작
		if(strpos($board['bo_skin'], 'pageMake') !== false) {
			$categorys = explode("|", $board['bo_category_list']);
			if(!$sca) $sca= $categorys[0];			
		}
		$ca_name = $sca ? $sca : '';
	}

	$is_notice = strpos($board['bo_skin'], 'pageMake') !== false ? false : true; //인태 - 공지가 없는 스킨을 위해서
	if(strpos($board['bo_skin'], 'shop_partners') !== false) $is_notice = false;
    $category_option = get_category_option($bo_table, $ca_name, $is_notice);
    $is_category = true;
}
...
if(G5_IS_MOBILE && $board['bo_use_dhtml_editor'] != '2') $is_dhtml_editor = false;  //인태 - 모바일은 에디터 사용 안함....(추후 수정)

$board['bo_editor_height'] = G5_IS_MOBILE && $board['bo_use_dhtml_editor'] != '2' ? 260 : $board['bo_editor_height']; //인태 - 모바일 내용쓰기 창 고정
$editor_html = editor_html('wr_content', $content, $is_dhtml_editor, $board['bo_editor_height']); //인태 - 에디터 높이 추가
$editor_js = '';
...
if($theme_type=='shop') {
	$is_back = true; //뒤로가기
	$head_title = $board['bo_subject'].' 글쓰기';
	$topMenu_skip = true;
}
...
include_once(G5_BBS_PATH.'/my/_write.php');