...
$mb_pass_chk = sql_fetch(" select mb_id from `g5_member` where mb_id = '".$mb_id."' and mb_password = '' ");
$mb_kakao_chk = sql_fetch(" select mb_id from `g5_member_social_profiles` where mb_id = '".$mb_id."' ");

if(!$mb_kakao_chk['mb_id']){
	if($mb_pass_chk['mb_id']){
		set_session('ss_tmp_mb_id', $mb_id);
		alert("사이트 리뉴얼로 인해 비밀번호 재설정이 필요합니다.", G5_URL."/tmp/form_password.php");
	}
}
...
<script>
	var jsonStr = '{ "type" : "login", "userId" : "<?=$mb_id?>" }';
