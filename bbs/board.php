...
if($theme_type=='shop' && !$board['bo_subject_hide']) {
	$head_title = $board['bo_subject'];
	if($group['gr_2'] && $group['gr_subject']) $head_title = $group['gr_subject'];
	$topMenu_skip = true;
...
echo '<div id="_boContainer" class="max-width">';
	...
	echo '<div id="_boContainer_con">';
		...
		if ($member['mb_level'] >= $board['bo_list_level'] && $board['bo_use_list_view'] || empty($wr_id)) {
			...
			if($board['bo_category_list'] && $board['bo_use_category'] && $board['bo_cate_all_hidden']) { //인태 - 분류사용시 전체분류를 사용하지 않는다면 기본페이지는 첫번째 분류페이지로.
				$firstpage = explode('|', $board['bo_category_list'])[0];
				if(!$sca) $sca = $firstpage;
			}
