...
if ($w == "") {
	...
	if($config['cf_use_stipulation']) { //[회원가입 약관사용여부]가 사용일때만 채크
		if ($config['cf_stipulation_label'] && (!isset($_POST['agree']) || !$_POST['agree'])) {
			alert($config['cf_stipulation_label'].'의 내용에 동의하셔야 회원가입 하실 수 있습니다.', G5_BBS_URL.'/register.php');
		}

		if ($config['cf_privacy_label'] && (!isset($_POST['agree2']) || !$_POST['agree2'])) {
			alert($config['cf_privacy_label'].'의 내용에 동의하셔야 회원가입 하실 수 있습니다.', G5_BBS_URL.'/register.php');
		}

		if ($config['cf_terms_label'] && (!isset($_POST['agree3']) || !$_POST['agree3'])) {
			alert($config['cf_terms_label'].'의 내용에 동의하셔야 회원가입 하실 수 있습니다.', G5_BBS_URL.'/register.php');
		}
	}
...
$_inc_file = $member_skin_path.'/register_form.skin.php';
