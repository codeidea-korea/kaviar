...
if($board['bo_use_category']) {
    ...
    if(!$ca_name && !$is_admin) { //인태 - 관리자는 분류없음 등록 가능 ────────────────────────
        $msg[] = '<strong>분류</strong>를 선택하세요.';
    } else {
        $categories = array_map('trim', explode("|", $board['bo_category_list'].($is_admin ? '|공지' : '')));
        if(!empty($categories) && !in_array($ca_name, $categories) && !$is_admin) //인태 - && !$is_admin 추가 (관리자는 분류없이 저장가능)
            $msg[] = '분류를 올바르게 입력하세요.';
...
if ($wr_subject == '' && !$is_admin) { //인태 - 게시판관리자는 제목없이 글작성 가능(필수입력 아닐때만) && !$is_admin
...
if ($wr_content == '' && $board['bo_use_html_tag'] != 'noCon' && !$is_admin) { //인태 - && $board['bo_use_html_tag'] != 'noCon' && !$is_admin
...
@include_once($board_pcskin_path.'/write_update.head.skin.php'); // 인태 - pc쪽 스킨을 공동 사용 ─────────────────────
...
if ($w == '' || $w == 'u') {
    ...
    if($w =='u' && $member['mb_id'] && $wr['mb_id'] === $member['mb_id'] && !$is_admin) { //인태 - 게시판관리자는 레벨상관없이 등록및 수정이 가능 && !$is_admin ─────────────────────
        ;
...
//인태 - 추가된 필드 ─────────────────────────────────────────────────────────
$wr_use = $_POST['wr_use'] ? $_POST['wr_use'] : '';
$wr_link_option = implode("|", $_POST['wr_link_option']);
$wr_btn1 = implode("|", $_POST['wr_btn1']);
$wr_btn2 = implode("|", $_POST['wr_btn2']);
$wr_btn3 = implode("|", $_POST['wr_btn3']);
$wr_btn4 = implode("|", $_POST['wr_btn4']);
$wr_btn5 = implode("|", $_POST['wr_btn5']);
$wr_btn6 = implode("|", $_POST['wr_btn6']);
$wr_btn1_color = implode("|", $_POST['wr_btn1_color']);
$wr_btn2_color = implode("|", $_POST['wr_btn2_color']);
$wr_btn3_color = implode("|", $_POST['wr_btn3_color']);
$wr_btn4_color = implode("|", $_POST['wr_btn4_color']);
$wr_btn5_color = implode("|", $_POST['wr_btn5_color']);
$wr_btn6_color = implode("|", $_POST['wr_btn6_color']);
...
if ($w == '' || $w == 'r') {
    ...
    if ($member['mb_id']) {
        ...
        $wr_password = '';
        $wr_email = addslashes(trim($_POST['wr_email']));
    ...
    $sql = " insert into $write_table
                set wr_num = '$wr_num',
					 ...
					 wr_use = '$wr_use',
                     wr_order = '$wr_order',
					 wr_subject_hide = '$wr_subject_hide',
					 wr_short_con = '$wr_short_con',
					 wr_content_mobile = '$wr_content_mobile',
					 wr_tag = '$wr_tag',
					 wr_link_target = '$wr_link_target',
					 wr_link_name = '$wr_link_name',
					 wr_link_option = '$wr_link_option',
					 wr_video = '$wr_video',
	...
	if($write_table == "g5_write_11_inquiry"){
		if($config['cf_manager_hp']){
			...
			aligo_sms_call("1:1문의가 등록되었습니다.", $receive_number, $send_number, "", "", "");
    ...
    // 새글 INSERT
	if(!$except_new_table) { //인태 - 새글에 제외할 스킨 설정(write_update.head.skin.php) ────────────────────────────────────────────────────────────────────────────
...
if(isset($_FILES['bf_file']['name']) && is_array($_FILES['bf_file']['name'])) {
    for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
        ...
        if (is_uploaded_file($tmp_file)) {
			...
			// ───────────────────────────────────────────────────────────────────────────────────────
			// 인태 - 이 부분부터는 세로사진 정상적으로 출력되도록 회전 수정하는 부분
...
//@include_once($board_skin_path.'/write_update.tail.skin.php');
 ...
 //인태 - pc쪽 스킨에서 인크루드 한다. ────────────────────────────────────────────────────────────────────────────────
@include_once($board_pcskin_path.'/write_update.skin.php');
@include_once($board_pcskin_path.'/write_update.tail.skin.php');
...
// 인태 - 상세팝업 사용중이거나, 업데이트 후 목록페이지로 이동이 필요한(상세페이지가 없는경우) 스킨은 저장후 목록페이지로 이동 ─────────────────────────────────────────────────
if($board['bo_layer_popup'] || $_POST['update_redirect_url'] == 'list') {
