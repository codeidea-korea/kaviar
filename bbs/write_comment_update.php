...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    if ($config['cf_email_use'] && $board['bo_use_email'])
    {
        ...
        // 1:1 문의일 때, 아래 설정 강제 - by ein1 260115
        if($bo_table == "11_inquiry") {

            // 보내는 메일 주소 - 관리자 페이지 > 홈페이지 기본환경 설정 > 관리자 메일 주소 설정 값 사용
            $wr_name = $config['cf_admin_email_name'];
            $wr_email = $config['cf_admin_email'];

            // 받는 메일 주소 - 1:1문의 글 남긴 사람의 이메일주소 사용
            $array_email[] = $wr['wr_email'];
        }
        // 1:1 문의일 때, 아래 설정 강제 - by ein1 260115 끝
