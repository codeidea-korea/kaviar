...
// 인태
if (file_exists(G5_THIS_PATH . '/bbs/register_form_update.php')) {
    require_once (G5_THIS_PATH . '/bbs/register_form_update.php');
    return;
}

$sns_type = isset($_POST['sns_type']) ? trim($_POST['sns_type']) : '';
...
$mb_code = isset($_POST['mb_code']) ? trim($_POST['mb_code']) : "";
if ($w == '' && $config['cf_req_membercode'] && $config['cf_use_membercode']) {
    $sql = " select * from {$g5['membercode_table']} where code_id = '$mb_code' ";
    $code = sql_fetch($sql);
...
$r1 = isset($_POST['r1']) ? trim($_POST['r1']) : '';
$r2 = isset($_POST['r2']) ? trim($_POST['r2']) : '';
$mb_etc = isset($_POST['mb_etc']) ? trim($_POST['mb_etc']) : '';
$mb_recomm = isset($_POST['mb_recomm']) ? trim($_POST['mb_recomm']) : '';
...
$mb_sns = isset($_POST['mb_sns']) ? trim($_POST['mb_sns']) : "";
...
$mb_births = $mb_year . "-" . $mb_month . "-" . $mb_day;
$mb_sexs = $r1;
$mb_buisness = ($r2 != '') ? "1" : "0";

if ($config['cf_use_nickname'] == 1) {
    $mb_name = $mb_nick;
} else {
    $mb_nick = $mb_name . "_" . date("dhis");
}
...
if ($w == '') {
    ...
    // 추천인 카운트 체크하기
    if ($mb_recomm) {
        $sqlr = " select mb_id, mb_recommend_cnt from {$g5['member_table']} where mb_id = '{$mb_recomm}' ";
        $rowr = sql_fetch($sqlr);
    }

    if ($rowr['mb_recommend_cnt'] < 6) {
        $recommend_cnt = $rowr['mb_recommend_cnt'] + 1;

        if ($rowr['mb_id']) {
            // 추천인 기입됫고 추천인이 있을때 insert 하기
            $sql_recommend = " insert into `g5_shop_member_recommend`
						set recommend = '{$mb_recomm}',
							recommender = '{$mb_id}',
							insertdate =  '" . G5_TIME_YMDHIS . "'     ";
            sql_query($sql_recommend);
        }
    } else {
        $recommend_cnt = $rowr['mb_recommend_cnt'];
    }
    ...
    $sql = " insert into {$g5['member_table']}
                set mb_id = '{$mb_id}',
					 ...
					 mb_births	  = '{$mb_births}',
					 mb_sexs	  = '{$mb_sexs}',
					 mb_buisness  = '{$mb_buisness}',
					 mb_etc		  = '{$mb_etc}',
					 mb_recommend_cnt = '{$recommend_cnt}',
                     mb_1 = '{$mb_1}',
                     mb_2 = '{$mb_2}',
                     mb_3 = '{$mb_3}',
                     mb_4 = '{$mb_4}',
    ...
    $object_sha = isset($_POST['object_sha']) ? trim($_POST['object_sha']) : '';
    $profile = isset($_POST['user_profile']) ? trim($_POST['user_profile']) : '';

    if ($provider) {

        $provider = strtolower($provider);

        $sql = sprintf("SELECT mp_no, mb_id from {$g5['social_profile_table']} where provider= '%s' and identifier = '%s' ", $provider, $identifier);
        ...
        $sql = sprintf("SELECT mp_no, object_sha, mp_register_day from {$g5['social_profile_table']} where mb_id= '%s' and provider= '%s' and identifier = '%s' ", $mb_id, $provider, $identifier);
...
if ($config['cf_member_img_size'] && $config['cf_member_img_width'] && $config['cf_member_img_height']) {
    ...
    $mb_tmp_dir .= substr($mb_id, 0, 2);

    // 회원 이미지 삭제
    if (isset($del_mb_img) && $del_mb_img)
        @unlink($mb_tmp_dir . '/' . $mb_icon_img);
    ...
    if (isset($_FILES['mb_img']) && is_uploaded_file($_FILES['mb_img']['tmp_name'])) {
        ...
        if (preg_match($image_regex, $_FILES['mb_img']['name'])) {
            ...
            if ($_FILES['mb_img']['size'] <= $config['cf_member_img_size']) {
                ...
                @mkdir($mb_tmp_dir, G5_DIR_PERMISSION);
                @chmod($mb_tmp_dir, G5_DIR_PERMISSION);
                $dest_path = $mb_tmp_dir . '/' . $mb_icon_img;
                ...
                if (file_exists($dest_path)) {
                    ...
                    if (! ($size[2] === 1 || $size[2] === 2 || $size[2] === 3 || $size[2] === 18)) { // gif jpg png webp 파일이 아니면 올라간 이미지를 삭제한다. // WebP 추가
                        ...
                        if ($size[2] === 2 || $size[2] === 3 || $size[2] === 18) { // WebP 추가
                                                                                   // jpg 또는 png 파일 적용
                            $thumb = thumbnail($mb_icon_img, $mb_tmp_dir, $mb_tmp_dir, $config['cf_member_img_width'], $config['cf_member_img_height'], true, true);
                            if ($thumb) {
                                @unlink($dest_path);
                                rename($mb_tmp_dir . '/' . $thumb, $dest_path);
                            }
                        }
                        if (! $thumb) {
                            // 아이콘의 폭 또는 높이가 설정값 보다 크다면 이미 업로드 된 아이콘 삭제
...
if ($w == '') {
	...
	$redirect_url = get_session('ss_redirect_urls');

    // 저장된 URL이 있고, 상품 상세 페이지였다면
    if($redirect_url && strpos($redirect_url, 'orderform.php') !== false) {
        // 세션 삭제
        set_session('ss_redirect_urls', '');
        // 해당 URL로 이동
