...
if ($stx) {
    ...
    for ($i=0; $i<count($g5_search['tables']); $i++) {
		...
		$board_result_count[$i] = $row['cnt'];//인태 - 추가
    }
    ...
    for ($idx=$table_index; $idx<count($search_table); $idx++) {
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
			...
			$list[$idx][$i]['name'] = '<div class="writer">'.get_mb_img($row['mb_id'],16).get_text(cut_str($row['wr_name'], $config['cf_cut_name'])).'</div>';
