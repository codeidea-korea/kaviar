...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if ($member['mb_level'] < $board['bo_read_level'] && !defined('_INDEX_')) { //인태 - 인텍스가 아닐때만 (게시판을 인텍스로 사용하기때문에)
        if ($is_member)
...
$width = $bo_viewpage && $board['bo_view_width'] ? $board['bo_view_width'] : $board['bo_table_width']; //인태 - 뷰페이지는 가로사이즈는 별도로...
if ($width <= 100)
    $width .= '%';
else
    $width .='px';
...
// ↑ 여기까지 bbs/board.php 에서 필요한 부분만 복사  ──────────────────────────

// ───────────────────────────────────────────────────────
// 인태 - 게시판 함수&옵션 정리 
include_once(G5_BBS_PATH.'/my/_board.php');
$is_magnific_popup = true;
include_once(G5_BBS_PATH.'/view.php');
// ───────────────────────────────────────────────────────


if(!G5_IS_MOBILE) {	
	$width = $board['bo_view_width'] ? $board['bo_view_width'] : $board['bo_table_width'];
	$width .= $width <= 100 ? '%;' : 'px;';

	$bo_popup_min_size = $board['bo_popup_min_size'] <= 100 ? $board['bo_popup_min_size'].'%;' : $board['bo_popup_min_size'].'px;';
	$bo_popup_max_size = $board['bo_popup_max_size'] <= 100 ? $board['bo_popup_max_size'].'%;' : $board['bo_popup_max_size'].'px;';
	$popSize = '';
	$popSize .= $board['bo_popup_min_size'] ? 'min-width:'.$bo_popup_min_size : '';
	$popSize .= $board['bo_popup_max_size'] ? 'max-width:'.$bo_popup_max_size : '';
	$pop_padding = $board['bo_popup_padding'] ? 'padding:'.$board['bo_popup_padding'].'px' :' ';
}