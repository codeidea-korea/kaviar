...
//가로 수
$_push_items_cols = '<div class="labelGroup itemsCols-label"><label class="labelInput"><span class="label">가로 수</span><input type="text" name="items_cols" value="'.($shopblock['items_cols']?$shopblock['items_cols']:'').'" class="w-75" placeholder="1"><span class="label label-inline">개씩</span></label><label class="labelInput"><span class="icon-mobile"></span><input type="text" name="items_cols_mobile" value="'.($shopblock['items_cols_mobile']?$shopblock['items_cols_mobile']:'').'" class="w-85" data-icon="mobile" placeholder="1"><span class="label label-inline">개씩</span></label></div>';
...
//썸네일 라운딩처리
$_push_items_radius = '<div class="labelGroup"><label class="labelInput"><span class="label">썸네일 라운딩처리</span><input type="text" name="items_radius" value="'.($shopblock['items_radius']?$shopblock['items_radius']:'').'" class="w-65" placeholder="0"><span class="label label-inline">PX</span></label><label class="labelInput"><span class="icon-mobile"></span><input type="text" name="items_radius_mobile" value="'.($shopblock['items_radius_mobile']?$shopblock['items_radius_mobile']:'').'" class="w-75" placeholder="0" data-icon="mobile"><span class="label label-inline">PX</span></label></div>';
...
$_push_banner_orderOption = '<div class="flex flex-middle gap0">'.$_push_banner_orderOption.$_push_items_count.$_push_btn_list_of_select.$_push_banner_orderOption2.'</div><div class="flex flex-middle gap25">'.$_push_items_cols.$_push_items_gap.'</div>';
...
$_push_item_orderOption .= '</select></label>';
$_push_item_orderOption = '<div class="flex flex-middle gap10">'.$_push_item_orderOption.$_push_items_count.$_push_btn_list_of_select.'</div><div class="flex flex-middle gap25">'.$_push_items_cols.$_push_items_gap.$_push_items_radius.'</div>';
		...
		<div class="form-list">
			...
			<div class="formCon flex column gap15">
				...
				<div class="flex flex-middle gap25">
					<input type="text" name="bl_width" value="<?=$shopblock['bl_width']?$shopblock['bl_width']:''?>" id="bl_width" class="w-70" data-label="가로사이즈" data-label-inline="PX" maxlength="4" placeholder="">					
		...
		<div id="blImgSet" class="form-list">
			...
			<div class="formCon flex flex-top gap60">
				<div class="banner_img">
					...
					if (file_exists($bl_img1)) {
						...
						$bl_img1_str .= '<label><input type="checkbox" name="del_bl_img1_del" value="1">삭제</label>';
					}
			...
			<div class="formCon flex flex-top gap60">
				<div class="banner_img">
					...
					if (file_exists($bl_img1_mobile)) {
						$bl_img2_str = '<img src="'.G5_DATA_URL.'/shop_block/bl'.$shopblock['bl_id'].'_2">';
						$bl_img2_str .= '<label><input type="checkbox" name="del_bl_img2_del" value="1">삭제</label>';
					}
						...
						echo '<li class="flex flex-middle">';
							...
							echo '<div class="fileImgSet" style="--img-size:80px">';
								echo '<div class="img_li">';
									...
									echo '</div>';
									if (file_exists($bl_icon_path[$i])) echo  '<label><input type="checkbox" name="del_bl_icon'.$i.'_del" value="1">삭제</label>';
								echo '</div>';
							echo '</div>';
...
function _bl_padding_placeholder(type) {
	...
	if(type == 'banner') {
		if(bl_title_val) {
			$('#bl_padding_t').attr('placeholder','45');
			$('#bl_padding_b').attr('placeholder','0');
			$('#bl_padding_lr').attr('placeholder','30');
		} else {
			$('#bl_padding_t, #bl_padding_b, #bl_padding_lr').attr('placeholder','0');
		}
		if(bl_width) {
		}
		if(bl_title_val || bl_title_mobile_val) {
			$('#bl_padding_mobile_t').attr('placeholder','25');
			$('#bl_padding_mobile_b').attr('placeholder','0');
			$('#bl_padding_mobile_lr').attr('placeholder','15');
		} else {
			$('#bl_padding_mobile_t, #bl_padding_mobile_b, #bl_padding_mobile_lr').attr('placeholder','0');
		}
	} else if(type == 'item') {
		if(bl_title_val) {
			$('#bl_padding_t').attr('placeholder','45');
			$('#bl_padding_b').attr('placeholder','35');
			$('#bl_padding_lr').attr('placeholder','30');
		} else {
			$('#bl_padding_t, #bl_padding_b, #bl_padding_lr').attr('placeholder','30');
		}
		if(bl_title_val || bl_title_mobile_val) {
			$('#bl_padding_mobile_t').attr('placeholder','25');
			$('#bl_padding_mobile_b').attr('placeholder','15');
			$('#bl_padding_mobile_lr').attr('placeholder','15');
		} else {
			$('#bl_padding_mobile_t, #bl_padding_mobile_b, #bl_padding_mobile_lr').attr('placeholder','15');
		}
		...
		if(bl_title_val || bl_title_mobile_val) {
			$('#bl_padding_mobile_t').attr('placeholder','25');
			$('#bl_padding_mobile_b').attr('placeholder','20');
			$('#bl_padding_mobile_lr').attr('placeholder','20');
		} else {
			$('#bl_padding_mobile_t, #bl_padding_mobile_b, #bl_padding_mobile_lr').attr('placeholder','20');
		}
	} else {
		if(bl_title_val) {
			$('#bl_padding_t').attr('placeholder','45');
...
//디폴트 값 (가로수)
function _items_cols_placeholder(type, items_skin) {
	if(type == 'banner') {
		$('input[name=items_cols]').attr('placeholder','1');
		$('input[name=items_cols_mobile]').attr('placeholder','1');
	} else if(type == 'item') {
		if(items_skin == '_slide') {
			$('input[name=items_cols]').attr('placeholder','3');
			$('input[name=items_cols_mobile]').attr('placeholder','1.25');
		} else if(items_skin == '_gall') {
			$('input[name=items_cols]').attr('placeholder','4');
			$('input[name=items_cols_mobile]').attr('placeholder','2');
		} else {
			$('input[name=items_cols]').attr('placeholder','3');
			$('input[name=items_cols_mobile]').attr('placeholder','1.25');
		}
		...
		if(items_skin == '_slide') {
			$('input[name=items_cols]').attr('placeholder','3');
			$('input[name=items_cols_mobile]').attr('placeholder','2.25');
		} else if(items_skin == '_gall') {
			$('input[name=items_cols]').attr('placeholder','4');
			$('input[name=items_cols_mobile]').attr('placeholder','2');
		} else {
...
function _items_gap_placeholder(type) {
	if(type == 'banner') {
		$('input[name=items_gap]').attr('placeholder','0');
		$('input[name=items_gap_mobile]').attr('placeholder','0');
	} else if(type == 'item') {
		$('input[name=items_gap]').attr('placeholder','30');
		$('input[name=items_gap_mobile]').attr('placeholder','15');
	} else if(type == 'itemuse') {
		$('input[name=items_gap]').attr('placeholder','30');
		$('input[name=items_gap_mobile]').attr('placeholder','15');
	}
...
function _select_item_option_push() {
	...
	optionContainer.find('select').selectpicker('refresh');
	_items_cols_placeholder('item', $('#items_skin').val());
	_items_gap_placeholder('item');
	matchOnOff('#items_skin', '_wz', '.itemsCols-label', 'hide');
	$('#label-item-outline, #label-item-shadow').show();
	$("#items_skin option[value='_wz']").prop('disabled',false);
	$('#items_skin').selectpicker('refresh');
}
...
function _select_itemuse_option_push() {
	...
	//_sel_item_option_list_of_();
	optionContainer.find('select').selectpicker('refresh');
	_items_cols_placeholder('itemuse', $('#items_skin').val());
	_items_gap_placeholder('itemuse');
	matchOnOff('#items_skin', '_wz', '.itemsCols-label', 'hide');
	$('#label-item-outline, #label-item-shadow').hide();
	$("#items_skin option[value='_wz']").prop('disabled',true);
	$('#items_skin').selectpicker('refresh');
}
...
$(document).ready(function(){	
	...
	matchOnOff(bl_type, '', '#blImgSet, #blVideoSet');
	...
	$('#items_skin').change(function (){
		_items_cols_placeholder('item', $(this).val());
	});
