...
<div id="find_info" class="new_win<?php if($config['cf_cert_use'] != 0 && $config['cf_cert_find'] != 0) { ?> cert<?php } ?>">
    <div class="new_win_con">
        ...
        <form method="post" name="fpasswordlost" action="<?php echo $action_url; ?>" onsubmit="return fpasswordlost_submit(this);" autocomplete="off">
            <h3>SMS로 찾기</h3>
            <fieldset id="info_fs">
                <p>
                    회원가입 시에 등록하신 휴대전화번호를 입력해주세요.<br />
                    해당 번호로 아이디와 비밀번호 정보를 보내드립니다.
                </p>
                <label for="mb_phone" class="sound_only">휴대전화번호<strong class="sound_only">필수</strong></label>
                <input type="text" name="mb_phone" id="mb_phone" class="required frm_input full_input" size="15" placeholder="휴대전화번호" required />
            </fieldset>
            <?php echo captcha_html(); ?>
            <div class="win_btn">
                <button type="submit" class="_btn/line/transparent/lg">SMS 보내기</button>
            </div>
        </form>
...
var phoneTest = /^(01[0]{1})[0-9]{3,4}[0-9]{4}$/;
function isPhoneTest(phone) {
    var phoneTest = /^(01[0]{1})[0-9]{3,4}[0-9]{4}$/;
...
function fpasswordlost_submit(f)
{
    ...
    for(var index = 0; index < phone.length; index++) {
        if(isPhoneTest(phone[index].value) == false) {
