...
if(file_exists(G5_THIS_SHOP_SKIN_PATH.'/item.form.skin.php')) {
	require_once(G5_THIS_SHOP_SKIN_PATH.'/item.form.skin.php');
	return;
}
					...
					if(file_exists($store_img_path)) {
						echo '<div id="sit_store">';
							if($store['store_url']) echo '<a href="'.$store['store_url'].'">';
								echo '<img src="'.get_url($store_img_url).'">';					
							if($store['store_url']) echo '</a>';
						echo '</div>';
					} else {						
						echo '<div id="sit_store">';
							if($store['store_url']) echo '<a href="'.$store['store_url'].'">';
								echo '<span class="tag">'.$store['store_subject'].'</span>';
							if($store['store_url']) echo '</a>';
						echo '</div>';
					}
				} ?>
