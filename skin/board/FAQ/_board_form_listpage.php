...
<form name="bf_form" id="bf_form" action="<?=G5_BBS_URL?>/my/_adm/_board_form_listpage_update.php" onsubmit="return bf_form_submit(this);" method="post" enctype="multipart/form-data">
...
echo '<label class="span"><input type="text" name="bo_category_label" value="'.$board['bo_category_label'].'" id="bo_category_label" class="frm_input span" style="font-weight:bold;" size="15" placeholder="카테고리"></label>';
echo '</div>';
...
<?php
echo '<div class="mt15 mb10">';
echo '<select name="bo_option[]" class="select-img n2 span140" data-id="gallStyle" data-label="">';
echo option_multiple_selected_my("",  '', "faq-list", "data-content=\"<img src='".get_url($board_skin_url."/img/faq-list.gif")."' alt='FAQ (리스트)'><span class='skin_name'>FAQ (리스트)</span>\"");
echo option_multiple_selected_my("faq-masonry",  $board['bo_option'], "faq-masonry", "data-content=\"<img src='".get_url($board_skin_url."/img/faq-masonry.gif")."' alt='FAQ (masonry)'><span class='skin_name'>FAQ (masonry)</span>\"");		
echo '</select>';
echo '</div>';
?>
...
<section class="mybox blue">
	<div class="formContainer label140">
		...
		<?php if(match_arr('faq-masonry', $board['bo_option'])) { ?>
		<div class="form-list">
	...
	if($autoSize) {			
		...
		echo '	</div>';
		echo '</div>';
		echo '<input type="hidden" name="bo_upload_count" value="2">';
	} else {
		echo '<input type="hidden" name="bo_gallery_cols" value="'.$board['bo_gallery_cols'].'">';
