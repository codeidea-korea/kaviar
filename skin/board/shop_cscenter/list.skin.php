...
if(!$is_admin) goto_url($write_href);

if (G5_IS_MOBILE) {
    include_once($board_skin_path.'/list.skin_mobile.php');
    return;
}
...
<div class="bo_list" style="<?=$bo_width?>">
    ...
    <div class="tableContainer">
        ...
        <tr class="<?php if($list[$i]['is_notice']) echo "bo_notice"; ?>">
			...
			<?php echo '<td class="td_state">'.(!$list[$i]['comment_cnt']?'<span class="state_tag ready">답변대기</span>':'<span class="state_tag complete">답변완료</span>').'</td>'; ?>
			<?php if($edit_href[$i]){
