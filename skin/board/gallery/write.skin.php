...
<section id="bo_w" style="<?=$bo_width?>">
    ...
    <div class="wr-wrap label130">
		<?=$wr_guest?>

		<?php if($wr_use || $wr_order || $wr_include) echo '<div class="wr-group sm:flex sm:gap10">'.$wr_use.$wr_order.$wr_include.'</div>'; ?>
		...
		<?php if($is_admin) {
			echo '<div class="wr-list">';
            echo '<div class="wr-list-con">';
			echo '<textarea name="wr_short_con" class="span autosize label" style="min-height:80px;" placeholder="목록페이지에 본문 대신 사용할 문구" data-label="목록페이지 문구">'.$write['wr_short_con'].'</textarea>';
			echo '</div>';
			echo '</div>';
		} ?>		
