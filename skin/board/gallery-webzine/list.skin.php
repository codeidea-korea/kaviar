...
<div class="bo_gall webzine boContainer" style="<?=$bo_width?>">
	...
	<div class="gall_ul">
		...
		<div class="gall_li <?=$skinOption_frame?> <?=$is_now[$i]?>">
			<?=$bo_current[$i]?>
			<?=$gall_li_checkbox[$i] ?>
			<?=$icon_use[$i]?>

			<div class="wzContents">

				<?php if($img[$i]) {
					echo '<div class="wz_thumb">';
					echo $a_link_img[$i];
					echo $img[$i];
				...
				<?php if($gall_con[$i] || $list_btn_set[$i]) { ?>
				<div class="wz_con">
			...
			<?php if($edit_href[$i]) {
				...
				echo '	<a href="'.$edit_href[$i].'" class="myTip mini right '.$includeOn[$i].'" data-tip="section_'.$list[$i]['wr_id'].'"><span class="btnEdit '.$btnEdit_class[$i].'">수정</span></a>';
				echo '</div>';
			} ?>
			<?//=$order_num[$i]?>
		</div>
