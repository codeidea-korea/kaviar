...
$result = sql_query(" SELECT * FROM g5_shop_order WHERE od_status = '주문' and unix_timestamp(od_time) < ".$t_day." " );
...
for ($i=0; $row=sql_fetch_array($result); $i++){
	...
	$od = sql_fetch(" select * from {$g5['g5_shop_order_table']} where od_id = '$od_id' and mb_id = '{$row['mb_id']}' ");
	...
	// 주문상품의 상태가 주문인지 체크
	//인태 - SUM(IF(ct_status = '입금', 1, 0)) as od_count3, 추가(240109)
	$sql = " select SUM(IF(ct_status = '주문', 1, 0)) as od_count2, SUM(IF(ct_status = '입금', 1, 0)) as od_count3,
					COUNT(*) as od_count1
	...
	if($od['od_cancel_price'] > 0 || $ct['od_count1'] != ($ct['od_count2']+$ct['od_count3'])) { //인태 - $ct['od_count3'] - 추가(240109)
	...
	if ($od['od_receipt_point'] > 0)
		insert_point($row['mb_id'], $od['od_receipt_point'], "주문번호 $od_id 본인 취소");
	...
	$sms_content14 = str_replace("{주문금액}", number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']), $sms_content14);
	...
	kakao_alim("TS_0387", $receive_number, $row['od_name'], $row['od_id'], "", "", "", "", "", "");
}
