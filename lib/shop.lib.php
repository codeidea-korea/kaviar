...
class item_list
{
	...
	//인태 - 추가
	protected $items_skin;
	protected $items_cols;
	protected $items_gap;
	protected $items_radius;
	protected $items_sel_li_id;
    ...
    function set_type($type) {
        //$this->type = $type;
		$this->type = $type == 'all' ? '' : $type; //인태 - 타입구분없이 전체상품 출력
        if ($type) {
	...
	// 지점코드를 인수로 넘기게 되면 해당 지점에 속한 상품을 노출합니다. - 인태
	...
	//인태 - 추가
	function set_items_skin($items_skin) {
        $this->items_skin = $items_skin;
    }
	function set_items_cols($items_cols) {
        $this->items_cols = $items_cols;
    }
	function set_items_gap($items_gap) {
        $this->items_gap = $items_gap;
    }
	function set_items_radius($items_radius) {
        $this->items_radius = $items_radius;
    }
	function set_items_sel_li_id($items_sel_li_id) {
        $this->items_sel_li_id = $items_sel_li_id;
    }
    ...
    function run() {
        ...
        global $g5, $config, $member, $default, $is_admin, $is_shop_manager, $is_closedmall; //인태 - $is_admin, $is_shop_manager
        ...
        if ($this->query) {
			...
			if($this->view_popular){//인기상품일경우

				if($_GET['ca_id'] == "all") {
					$where_ca_ids .= " ca_id like '%' ";
					if ($_GET['ca_id2']) {
						
						$carrs = explode("_",$_GET['ca_id2']);
						...
						if(count($carrs) > 1){
							...
							$where_ca_ids .= " and ( ca_id2 in (".$inner.") or ca_id3 in (".$inner.") ) ";
						}else{
							$where_ca_ids .= " and (ca_id2 = '".$_GET['ca_id2']."' or ca_id3 = '".$_GET['ca_id2']."') ";
						}
					...
					$where_ca_ids .= " ( ca_id = '".$_GET['ca_id']."' or ca_id2 = '".$_GET['ca_id']."' or ca_id3 = '".$_GET['ca_id']."' )";
				...
				if($_GET['price'] != "") {
					...
					if($_GET['price'] == 1){
						$where_ca_ids .= " and it_price < 6000 ";
					}else if($_GET['price'] == 2){
						$where_ca_ids .= " and it_price > 5999 and it_price < 10001";
					}else if($_GET['price'] == 3){
						$where_ca_ids .= " and it_price > 9999 and it_price < 20001";
					}else if($_GET['price'] == 4){
						$where_ca_ids .= " and it_price > 19999 ";
					}
				...
				if($_GET['tags'] != "") {
					...
					$where_ca_ids .= " and it_tag like '%".$_GET['tags']."%' ";
				...
				if ($this->ca_id || $this->ca_id2 || $this->ca_id3) {
					$where_ca_id = array();
					if ($this->ca_id) {
						$ca_id1_arr = explode(",", $this->ca_id);
						for ($c1=0; $c1<count($ca_id1_arr); $c1++) {
							$ca_id1 = trim($ca_id1_arr[$c1]);
							if($ca_id1=='')
								continue;
							$where_ca_id1[] = "ca_id like '{$ca_id1}%' " ;
						}
						$where_ca_id[] = " ( " . implode(" or ", $where_ca_id1) . " ) ";
						//$where_ca_id[] = " ca_id like '{$this->ca_id}%' ";
					}
					if ($this->ca_id2) {
						$ca_id2_arr = explode(",", $this->ca_id2);
						for ($c2=0; $c2<count($ca_id2_arr); $c2++) {
							$ca_id2 = trim($ca_id2_arr[$c2]);
							if($ca_id2=='')
								continue;
							$where_ca_id2[] = "ca_id2 like '{$ca_id2}%' " ;
						}
						$where_ca_id[] = " ( " . implode(" or ", $where_ca_id2) . " ) ";
						//$where_ca_id[] = " ca_id2 like '{$this->ca_id2}%' ";
					}
					if ($this->ca_id3) {
						$ca_id3_arr = explode(",", $this->ca_id3);
						for ($c3=0; $c3<count($ca_id3_arr); $c3++) {
							$ca_id3 = trim($ca_id3_arr[$c3]);
							if($ca_id3=='')
								continue;
							$where_ca_id3[] = "ca_id3 like '{$ca_id3}%' " ;
						}
						$where_ca_id[] = " ( " . implode(" or ", $where_ca_id3) . " ) ";
			...
			if ($this->items_sel_li_id) {	
				...
				for ($t=0; $t<count($sel_li_ids); $t++) {
					...
					if($sel_li_id=='')
						continue;
					$where_it_id[] = 'it_id = '.$sel_li_id.'';
				}
				$where[] = " ( " . implode(" or ", $where_it_id) . " ) ";
			}
			/////////////////////////////////////////
			...
			} else if ($this->store) { //인태 - 지점 상품 출력
                $sql_select = " select {$this->fields} ";
                $sql_common = " from `{$g5['g5_shop_store_item_table']}` a left join `{$g5['g5_shop_item_table']}` b on (a.it_id = b.it_id) ";
                $where[] = " a.store_id = '{$this->store}' ";
            } else {
                $sql_select = " select {$this->fields} ";
			...
			if(G5_IS_MOBILE){
				$sql_limit = " limit " . $this->from_record . " , 20";
			}else{
	            //$sql_limit = " limit " . $this->from_record . " , " . ($this->list_mod * $this->list_row);
				$sql_limit = " limit " . $this->from_record . " , 40";
			}
			//$sql_limit = " limit 0,8";
...
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=true) //인태 - $is_crop=true
{
    global $g5, $is_closedmall;
	...
	for($i=1;$i<=10; $i++) {
		...
		if(is_file($file) && $row['it_img'.$i]) {
			...
			if( (! isset($size[2]) || $size[2] < 1 || $size[2] > 3 ) && !file_ext_type($row['it_img'.$i]) == 'video') //인태 - 상품이미지가 동영상이라면
				continue;
	...
	if($filename && file_ext_type($filename) == 'video') { //인태 - 상품이미지가 동영상이라면

		$video_file = G5_DATA_URL.'/item/'.$row['it_id'].'/'.$filename;
		$video = '<video src="'.$video_file.'" autoplay muted loop class="video" style="max-width:100%;'.($width?'width:'.$width.'px;':'').'">﻿</video>';
		...
		if($thumb) {
			...
			if($is_closedmall) $img .= ' class="blur"';
		} else {
...
function get_it_thumbnail($img, $width, $height=0, $id='', $is_crop=true) //인태 - $is_crop=true
{
    $str = '';
	
	//인태 - 업로드 파일이 동영상이라면
	if(file_ext_type($img) == 'video') {
		
		$file = G5_DATA_URL.'/item/'.$img;
		$str = '<video src="'.$file.'" autoplay muted loop class="video" style="max-width:100%;">﻿</video>';
...
function get_option_stock_qty($it_id, $io_id, $type)
{
    ...
    $row = sql_fetch($sql);
    $daegi = (int)$row['sum_qty'];
if($_SERVER['REMOTE_ADDR']=='59.15.184.21'){
...
// 금액 표시 - 배송비
function display_price1($price, $tel_inq=false)
{	
	global $member;
...
// 금액표시
// $it : 상품 배열
function get_origin_price($it_id)
{
	global $g5;
...
function it_img_upload($srcfile, $filename, $dir)
{
	...
	if (!preg_match('/\.(mp4)$/i', $filename)) { //인태 - 동영상 업로드 추가
		//mp4 파일 업로드시 이미지타입 검사 패스
		$size = @getimagesize($srcfile);	
		if($size[2] < 1 || $size[2] > 3)
			return '';
	}
    ...
    if (!preg_match('/\.(gif|jpe?g|png|webp|mp4)$/i', $filename)) //인태 - mp4 추가
        return '';
...
function get_item_options($it_id, $subject, $is_div='', $is_first_option_title='')
{
    ...
    if($subj_count > 1) {
		...
		$first_option_title = $is_first_option_title ? $subject.'를 선택해주세요' : '선택';
...
function display_banner($position, $skin='') {
	...
	if(!$skin) $skin = 'slidebanner.skin.php';
...
function get_item_point1($it, $io_id='',$it_prices, $qty, $trunc=100)
{
    global $g5;
...
// 상품포인트
function get_item_point_renew($it, $io_id='',$mb_id='', $trunc=10)
{
    global $g5;

    $it_point = 0;
	
	$mbs = sql_fetch("select mb_grade from `g5_member` where mb_id = '".$mb_id."' ");
	$grade = sql_fetch(" select g_reward from `g5_member_grade` where g_type = 'Y' and idx = ".$mbs['mb_grade']." ");
    ...
    if($it['it_point_type'] > 0) {
		...
		if($opt['it_grade'] == 1 && $config['cf_grade'] == 1){ //회원혜택을 사용하면서 회원해택 config Y인경우

			if($grade['g_reward'] != '0'){

				if($row['io_type'] == 1){ //추가상품일경우
					$po_point = $row['io_price'] * ($grade['g_reward'] / 100 );
				}else{ //추가상품이 아닐경우
					$po_point = ($row['io_price'] + $row['ct_price']) * ($grade['g_reward'] / 100 );
					
				}

				$po_point =round($po_point/100)*100;
...
function check_itemuse_write($it_id, $mb_id, $close=true)
{
    ...
    if(!$is_admin && $default['de_item_use_write'])
    {
        ...
        {
            if($close) {
				echo '<script>alert("사용후기는 주문이 완료된 경우에만 작성하실 수 있습니다."); self.close();</script>'; //인태 수정
...
//적립금 준섭 사용안함
function save_order_point_new($ct_status="완료")
{
    global $g5, $default, $config;
	

	//원래는 상품별로 부여되는 포인트로 적립이되는데 이걸 변경해서 등급별로 바꿈.
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        // 회원 ID 를 얻는다.
        $od_row = sql_fetch("select od_id, mb_id from {$g5['g5_shop_order_table']} where od_id = '{$row['od_id']}' ");
		$mbs = sql_fetch("select mb_grade from `g5_member` where mb_id = '{$od_row['mb_id']}' ");
		$grade = sql_fetch(" select g_reward from `g5_member_grade` where g_type = 'Y' and idx = ".$mbs['mb_grade']." ");
		
		$its = sql_fetch(" select it_grade from `g5_shop_item` where it_id = '".$row['it_id']."' ");
        //if ($od_row['mb_id'] && $row['ct_point'] > 0) { // 회원이면서 포인트가 0보다 크다면
		
		if($its['it_grade'] == 1){

			if($config['cf_grade'] == 1){
				...
				if($grade['g_reward'] != '0'){
					...
					$po_point =floor($po_point/100)*100;
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++){
       ...
       /* if( $it['it_price'] !== $row['ct_price'] ){
        ...
        if($config['cf_use_point']) {
            ...
            if($row['io_type'] == 0) {
                $compare_point = get_item_point1($it, $row['io_id'],$it_price,$row['ct_qty']);
            } else {
                //$compare_point = $it['it_supply_point'];
				$compare_point = get_item_point1($it, $row['io_id'],'A',$row['ct_qty']);
            }
...
//등급별 금액차등
function get_grade_price($it_price){

    global $member,$config;

	if($config['cf_grade'] == 1){

		$sql = " select g_discount from `g5_member_grade` where g_type = 'Y' and idx = ".$member['mb_grade']." ";
		$row = sql_fetch($sql);
		
		if($row['g_discount'] != '0'){
			$it_price = $it_price - ($it_price * ($row['g_discount'] / 100 ));
			$it_price =round($it_price/100)*100;
		}
	}
