...
// 게시글리스트 썸네일 생성
function get_list_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3', $option=0, $editor_img=true) //인태 - $option=0, $editor_img=true 추가
{
    global $g5, $config;
    $filename = $alt = $data_path = '';
    $edt = false;

    $row = get_thumbnail_find_cache($bo_table, $wr_id, 'file', $option); //인태 - $option 추가
    ...
    if(isset($row['bf_file']) && $row['bf_file']) {
        ...
        $write = get_thumbnail_find_cache($bo_table, $wr_id, 'content', $option); //인태 - $option 추가
        $edt = true;
        ...
        if( $matches = get_editor_image($write['wr_content'], false) ){
            for($i=0; $i<count($matches[1]); $i++)
            {
				...
				if($editor_img) $p = parse_url($matches[1][$i]); //인태 - if($editor_img)
    ...
    $tname = thumbnail($filename, $filepath, $filepath, $thumb_width, $thumb_height, $is_create, $is_crop, $crop_mode, $is_sharpen, $um_value, $option); //인태 - $option 추가
...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3', $option='0') //인태 - $option='0' 추가
{
    global $g5;
