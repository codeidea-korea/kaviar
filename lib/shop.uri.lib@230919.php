...
//이벤트 짧은 주소 - 인태 추가
function shop_event_url($ev_id, $add_param=''){
    global $config;

    if( $config['cf_bbs_rewrite'] ){
        return get_pretty_url('shop', 'ev-'.$ev_id, $add_param);
    }
	
	$add_params = $add_param ? '&'.$add_param : '';
    return G5_SHOP_URL.'/event.php?ev_id='.urlencode($ev_id).$add_params;
}
...
function add_pretty_shop_url($url, $folder, $no='', $query_string='', $action=''){
    ...
    if( $config['cf_bbs_rewrite'] ){
        ...
        if( $config['cf_bbs_rewrite'] > 1 && ! preg_match('/^(list|type|ev)\-([^\/]+)/i', $no) ){
		...
		} else if( preg_match('/^ev\-([^\/]+)/i', $no) ){ //인태 추가
            $url = G5_SHOP_URL. '/event.php?ev_id='.urlencode($no);
        } else {
...
function shop_short_url_clean($string_url, $url, $page_name, $array_page_names){
	...
	if( $config['cf_bbs_rewrite'] && stripos($string_url, G5_SHOP_URL) !== false && in_array($page_name, array('item', 'list', 'listtype', 'ev')) ){ //인태 추가 - ev
		
		parse_str($url['query'], $vars);
		
		$allow_param_keys = array('it_id'=>'', 'ca_id'=>'', 'type'=>'', 'ev_id'=>'');
        ...
        foreach( $allow_param_keys as $key=>$v ){
			...
			} else if ( $key === 'ev' ){ //인태 추가
				$key_value = 'ev-'.$vars[$key];
			}
        ...
        if( isset($url['host']) ){
            ...
            $array_file_paths = run_replace('url_clean_page_paths', array('/'.G5_SHOP_DIR.'/item.php', '/'.G5_SHOP_DIR.'/list.php', '/'.G5_SHOP_DIR.'/listtype.php', '/'.G5_SHOP_DIR.'/event.php')); //인태 추가 - event.php
...
function add_shop_nginx_conf_rules($rules, $get_path_url, $base_path, $return_string=false){
	...
	$add_rules[] = "rewrite ^{$base_path}shop/ev-([0-9a-z]+)$ {$base_path}".G5_SHOP_DIR."/event.php?ev_id=$1&rewrite=1 break;"; //인태 추가
...
function add_shop_mod_rewrite_rules($rules, $get_path_url, $base_path, $return_string=false){
	...
	$add_rules[] = 'RewriteRule ^shop/ev-([0-9a-z]+)$  '.G5_SHOP_DIR.'/event.php?ev_id=$1&rewrite=1  [QSA,L]'; //인태 추가
