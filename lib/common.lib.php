...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
	...
	*/
	...
	if ($board['bo_use_sideview']) {
        ...
        $list['name'] = '<span class="'.($list['mb_id']?'sv_member':'sv_guest').'">'.$tmp_name.'</span>';
		$mb_img_size = G5_IS_MOBILE ? 17 : 20;
		$list['writer'] = '<span class="writer">'.get_mb_img($list['mb_id'], $mb_img_size, $mb_img_size, false).$tmp_name.'</span>';
	}
	/* ───────────────────────────────────────────────── */
...
function get_category_option($bo_table='', $ca_name='', $is_notice=true)
{
    global $g5, $board, $is_admin;

    //$categories = explode("|", $board['bo_category_list'].($is_admin?"|공지":"")); // 구분자가 | 로 되어 있음
	$categories = $is_notice ? explode("|", $board['bo_category_list'].($is_admin?"|공지":"")) : explode("|", $board['bo_category_list']); // 인태 - 공지없는 스킨을 위해서..
    $str = "";
    ...
    if ($mb_id) {
        ...
        if ($config['cf_use_member_icon']) {
            ...
            if (file_exists($icon_file)) {
                ...
                if ($config['cf_use_member_icon']) // 회원아이콘+이름
...
function member_delete($mb_id)
{
	...
	//인태 - 디렉토리내 모든 파일 삭제 --------------
	$delete_path = G5_DATA_PATH.'/member/'.$mb_id;	
	rm_rf($delete_path); //인태 - 디렉토리 삭제
	//--------------------------------------------------
...
function validateString($string) {
    $pattern = '/^(?:(?:[a-zA-Z]+[0-9]+)|(?:[가-힣]+[0-9]+)|(?:[a-zA-Z]+[가-힣]+)|[0-9]+|[가-힣]+|[a-zA-Z]+)$/u';
...
function insert_popular($field, $str)
{
    ...
    if(!in_array('mb_id', $field)) {
		if(validateString($str)){
			...
			$sql = " insert into {$g5['popular_table']} set pp_word = '{$str}', pp_date = '".G5_TIME_YMDHIS."', pp_ip = '{$_SERVER['REMOTE_ADDR']}', session_id = '{$session_id}' ";
			sql_query($sql, FALSE);
		}
    }
}
...
function get_skin_path($dir, $skin)
{
    ...
    if(preg_match('#^theme/(.+)$#', $skin, $match)) { // 테마에 포함된 스킨이라면
        ...
        if(G5_IS_MOBILE) {
			...
			if(!is_dir($skin_path))
                $skin_path = G5_SKIN_PATH.'/'.$dir.'/'.$skin; //인태 - 추가
        } else {
