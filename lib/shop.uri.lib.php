...
//쇼핑몰 짧은 주소 - 인태 추가
function shop_short_url_my($pn, $no='', $add_param=''){
    global $config;

    if( $config['cf_bbs_rewrite'] ){
		if($pn) {
			//return get_pretty_url('shop', 'ev-'.$ev_id, $add_param);
			return get_pretty_url('shop/'.$pn, $no, $add_param);
		} else {
			return G5_SHOP_URL;
		}
    }
	...
	if($no) {
		if($pn == 'event') $de_param = 'ev_id';
		if($pn == 'page') $de_param = 'pn_id';
		$add_params = $add_param ? '&'.$add_param : '';
		return G5_SHOP_URL.'/'.$pn.'.php?'.$de_param.'='.urlencode($no).$add_params;
	} else {
		$add_params = $add_param ? '?'.$add_param : '';
		return G5_SHOP_URL.'/'.$pn.'.php'.$add_params;
	}
}
...
function add_shop_nginx_conf_rules($rules, $get_path_url, $base_path, $return_string=false){
    ...
    $add_rules[] = "rewrite ^{$base_path}shop/shopCate {$base_path}".G5_SHOP_DIR."/shopCate.php?rewrite=1 break;"; //인태 추가
    $add_rules[] = "rewrite ^{$base_path}shop/shopStore {$base_path}".G5_SHOP_DIR."/shopStore.php?rewrite=1 break;"; //인태 추가
...
function add_shop_mod_rewrite_rules($rules, $get_path_url, $base_path, $return_string=false){
	...
	$add_rules[] = 'RewriteRule ^shop/shopCate '.G5_SHOP_DIR.'/shopCate.php?rewrite=1  [QSA,L]'; //인태 추가
	$add_rules[] = 'RewriteRule ^shop/shopStore '.G5_SHOP_DIR.'/shopStore.php?rewrite=1  [QSA,L]'; //인태 추가
