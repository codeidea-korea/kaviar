...
function get_board_file_db($bo_table, $wr_id, $fields='*', $add_where='', $option='0', $is_cache=false) //인태 - $option='0' 추가
{
    global $g5;
    ...
    $wr_id = (int) $wr_id;
    $key = md5($bo_table.'|'.$wr_id.$fields.$add_where.$option); //인태 - $option 추가
	...
	$sql = " select $fields from {$g5['board_file_table']}
                where bo_table = '$bo_table' and wr_id = '$wr_id' $add_where and bf_no = $option ";  //인태 - 번호가 맞는 첨부이미지만 불러오기
...
function get_thumbnail_find_cache($bo_table, $wr_id, $wr_key, $option){ //인태 - $option 추가
    global $g5;
    ...
    return get_board_file_db($bo_table, $wr_id, 'bf_file, bf_content', "and bf_type in (1, 2, 3, 18) ", $option, true); //인태 - $option 추가
}
